...
public class ReturnMerchandise_PartialReturnOfItems_Check extends Browser_Factory{
	...
	{
		return Excel.getCellValue_inlist(path, "Merchandise","Merchandise","ReturnMerchandise_PartialReturnOfItems_Check");
	}
	...
	{
		try{
			logger=extent.startTest("ReturnMerchandise_PartialReturnOfItems_Check", "ReturnMerchandise_PartialReturnOfItems_Check");
	...
	/*		
		String Query="Select top 1 max(X.ReceiptNumber) as receiptnumber,X.accountorderid, X.accountnumber, sum(X.MerchandiseReturn) "+
			...
			if(saletax.equals(salesTax)){
				...
				logger.log(LogStatus.INFO, "The salestax  in UI : "+salesTax+" and DB : "+saletax+" are same",image);
			}
			else{
				  scpath=Generic_Class.takeScreenShotPath();
				Reporter.log(scpath,true);
				...
				logger.log(LogStatus.INFO, "The salestax  in UI : "+salesTax+" and DB : "+saletax+" are not same",image);
			}
	...
	public void afterMethod(){
		...
		if(resultFlag.equals("pass")){
			Excel.setCellValBasedOnTcname(path, "Merchandise","ReturnMerchandise_PartialReturnOfItems_Check" , "Status", "Pass");

		}else if (resultFlag.equals("fail")){

			Excel.setCellValBasedOnTcname(path, "Merchandise","ReturnMerchandise_PartialReturnOfItems_Check" , "Status", "Fail");
		}else{
			Excel.setCellValBasedOnTcname(path, "Merchandise","ReturnMerchandise_PartialReturnOfItems_Check" , "Status", "Skip");
		}
