...
public class ReturnMerchandise_Customer_WhoPaidBy_Check extends Browser_Factory{
	...
	{
		return Excel.getCellValue_inlist(path, "Merchandise","Merchandise","ReturnMerchandise_Customer_WhoPaidBy_Check");
	}
	...
	{
		try{
			logger=extent.startTest("ReturnMerchandise_Customer_WhoPaidBy_Check", "ReturnMerchandise_Customer_WhoPaidBy_Check");
			...
			/*String Query = ""
					+ " Select top 10 max(X.ReceiptNumber) as receiptnumber,X.accountorderid, X.accountnumber,X.Quantity, sum(X.MerchandiseReturn) "
					+ ",  sum(X.Creditcard) "
					+ ", sum(X.Cash) "
					+ ",Sum(X.Cheque) "
					+ "From "
					+ " (select A.accountnumber, AO.accountorderid,aoi.Quantity, case when t.name='Merchandise Sale' then CLM.CLmastergroupID end as ReceiptNumber, "
					...
					+ " group by X.accountorderid, X.accountnumber, X.Quantity "
					+ " having sum(X.MerchandiseReturn)=0 "
					+ " and sum(X.Creditcard)=0 "
					+ " and sum(X.Cash)=0 "
					+ " and Sum(X.Cheque)>=1";   */
			String Query=""
					+ "Select top 1 max(X.ReceiptNumber) as receiptnumber,X.accountorderid, X.accountnumber,x.posteddatetime, sum(X.MerchandiseReturn) "
					+ "     ,  sum(X.Creditcard), sum(X.Cash),Sum(X.Cheque) From "
					+ "     (select A.accountnumber, AO.accountorderid,clm.posteddatetime, case when t.name='Merchandise Sale' then CLM.CLmastergroupID end as ReceiptNumber, "
					+ "     t.name as CltransactionType, t1.name as paymenttype, clt.amount+Clt.discountamount as Amount, "
					+ "     case when t1.name='Credit Card' then 1 else 0 end as Creditcard, "
					+ "     case when t1.name='Cash' then 1 else 0 end as Cash, "
					+ "     case when t1.name='Check' then 1 else 0 end as Cheque, "
					...
					+ "     left join clpayment CLP with(nolock) on CLP.Cltransactionmasterid=CLM.Cltransactionmasterid "
					+ "     left join type T1 with(nolock) on T1.typeid=CLP.PaymentSourceTypeID "
					+ "     where Datediff (dd, clm.posteddatetime, getutcdate())<30)  X "
					+ "     group by X.accountorderid, X.accountnumber,x.posteddatetime "
					+ "     having sum(X.MerchandiseReturn)=0 "
					+ "     and sum(X.Creditcard) =0 "
					+ "     and sum(X.Cash)=0 "
					+ "     and Sum(X.Cheque)>=1 "
					+ "    and posteddatetime < getutcdate()-1 "
			...
			//pmhomepage.enter_Receiptnumber("611164993");
			...
			logger.log(LogStatus.INFO, "selected required return quantity");*/
			//==========================================
			List<WebElement> lst=driver.findElements(By.xpath("//div[@id='merchandiseCart']//table/tbody/tr"));
			System.out.println(lst.size());
			for(int j=1;j<=lst.size();j++){
				String quant=driver.findElement(By.xpath("//div[@id='merchandiseCart']//table/tbody/tr["+j+"]/td[5]")).getText();
				int quantity1=Integer.parseInt(quant);
				System.out.println(quantity1);
				for(int i=1;i<=quantity1;i++){
					/*WebElement count=driver.findElement(By.xpath("div[@id='merchandiseCart']//table/tbody/tr["+j+"]/td[7]//a[@class='js-add psbutton-low-priority qtyicon floatleft']"));
				count.click();*/
					merchreturn.click_addQuantity_ofallitems(j);
				}
				//j++;
			}
			...
			String ActretSubtotal=("$"+ActretSubtotal1).substring(0, 3);;;
			...
			String expSubtotal=("$"+ExpRetSubtotal).substring(0, 3);;;
			...
			String current_siteid =DataBase_JDBC.executeSQLQuery( "select SiteID from siteparameter where paramcode like 'IP_COMPUTER_FIRST' and paramvalue='"
					+ ipadd + "'");
			...
			String taxquery="select s.taxpercent, s.* from sitetax s "+
					...
					" where s.siteid="+current_siteid+ "and t.name='Merchandise' and s.expirationdate is null ";
			...
			{
				logger.log(LogStatus.PASS,"Alert popup is not visible exception"+e);
			}
	...
	public void afterMethod(){
		...
		if(resultFlag.equals("pass")){
			Excel.setCellValBasedOnTcname(path, "Merchandise","ReturnMerchandise_Customer_WhoPaidBy_Check" , "Status", "Pass");

		}else if (resultFlag.equals("fail")){

			Excel.setCellValBasedOnTcname(path, "Merchandise","ReturnMerchandise_Customer_WhoPaidBy_Check" , "Status", "Fail");
		}else{
			Excel.setCellValBasedOnTcname(path, "Merchandise","ReturnMerchandise_Customer_WhoPaidBy_Check" , "Status", "Skip");
		}
