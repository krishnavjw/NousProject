...
public class ReturnMerchandise_Customer_WhoPaidBy_CreditCard extends Browser_Factory{
	...
	{
		return Excel.getCellValue_inlist(path, "Merchandise","Merchandise","ReturnMerchandise_Customer_WhoPaidBy_CreditCard");
	}
	...
	{
		try{
			logger=extent.startTest("ReturnMerchandise_Customer_WhoPaidBy_CreditCard", "ReturnMerchandise_Customer_WhoPaidBy_CreditCard");
			...
			String Query="Select top 1 max(X.ReceiptNumber) as receiptnumber,X.accountorderid, X.accountnumber, sum(X.MerchandiseReturn) "+
					...
					"having sum(X.MerchandiseReturn)=0 -- Not yet returned "+
					"and sum(X.Creditcard)>=1 -- creditcard "+
					"and sum(X.Cash)=0 "+
					"and Sum(X.Cheque)=0  ";
...
Thread.sleep(6000);
	...
	public void afterMethod(){
		...
		if(resultFlag.equals("pass")){
			Excel.setCellValBasedOnTcname(path, "Merchandise","ReturnMerchandise_Customer_WhoPaidBy_CreditCard" , "Status", "Pass");

		}else if (resultFlag.equals("fail")){

			Excel.setCellValBasedOnTcname(path, "Merchandise","ReturnMerchandise_Customer_WhoPaidBy_CreditCard" , "Status", "Fail");
		}else{
			Excel.setCellValBasedOnTcname(path, "Merchandise","ReturnMerchandise_Customer_WhoPaidBy_CreditCard" , "Status", "Skip");
		}
