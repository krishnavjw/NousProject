...
public class CancelReservation_Business_ExistingCustomer extends Browser_Factory {
	...
	public void afterMethod() {
		...
		if (resultFlag.equals("pass")) {
			Excel.setCellValBasedOnTcname(path, "WalkinReservationLease", "CancelReservation_Business_ExistingCustomer",
					"Status", "Pass");
	...
	public void CancelReservation_Business_ExistingCustomer(Hashtable<String, String> tabledata) throws InterruptedException {
		logger = extent.startTest("CancelReservation_Business_ExistingCustomer",
				"Cancel an Existing Customer Reservation - Business");
		testcaseName = tabledata.get("TestCases");
		Reporter.log("Test case started: " + testcaseName, true);
		if (!(tabledata.get("Runmode").equals("Y") && tabledata.get("WalkinReservationLease").equals("Y"))) {
			resultFlag = "skip";
			logger.log(LogStatus.SKIP, "CancelReservation_Business_ExistingCustomer is Skipped");
		...
		try {
			...
			String resid = "select top 1 reservationid from reservation r"
					+ " join account A on A.accountid=r.reservationaccountid"
					...
					+ " and reservationstatustypeid in ('127', '128')" + " order by NEWID()";
	...
	public Object[][] getLoginData() {
		...
		return Excel.getCellValue_inlist(path, "WalkinReservationLease", "WalkinReservationLease",
				"CancelReservation_Business_ExistingCustomer");
	}
