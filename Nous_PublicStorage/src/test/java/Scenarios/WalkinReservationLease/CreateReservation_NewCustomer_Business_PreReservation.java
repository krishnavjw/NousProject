...
public class CreateReservation_NewCustomer_Business_PreReservation extends Browser_Factory {
	...
	public void afterMethod() {
		...
		if (resultFlag.equals("pass")) {
			...
			Excel.setCellValBasedOnTcname(path, "WalkinReservationLease",
					"CreateReservation_NewCustomer_Business_PreReservation", "Status", "Pass");

		} else if (resultFlag.equals("fail")) {

			Excel.setCellValBasedOnTcname(path, "WalkinReservationLease",
					"CreateReservation_NewCustomer_Business_PreReservation", "Status", "Fail");
		} else {
			Excel.setCellValBasedOnTcname(path, "WalkinReservationLease",
					"CreateReservation_NewCustomer_Business_PreReservation", "Status", "Skip");
		}
	...
	public void CreateReservation_NewCustomer_Business_PreReservation(Hashtable<String, String> tabledata)
		...
		try {
			...
			logger = extent.startTest("CreateReservation_NewCustomer_Business_PreReservation",
			...
			DataBase_JDBC.executeSQLQuery(
					"update siteparameter set ParamValue=15 where paramcode='max_reservation_days' and siteid=(select SiteID from Site where SiteNumber='"
							+ sitenum + "')");
			logger.log(LogStatus.INFO, "Setting the ReservationDeposit = 0");

			DataBase_JDBC.executeSQLQuery(
					"update siteparameter set ParamValue='0' where paramcode='ReservationDepositEnabled' and siteid=(select SiteID from Site where SiteNumber='"
							+ sitenum + "')");
			...
			logger.log(LogStatus.INFO, "Reservation is created", image);
			Thread.sleep(2000);
			// generics.Page_ScrollDown();
			((JavascriptExecutor) driver).executeScript("window.scrollTo(0, document.body.scrollHeight)");
			Thread.sleep(2000);
	...
	public Object[][] getData() {
		...
		return Excel.getCellValue_inlist(path, "WalkinReservationLease", "WalkinReservationLease",
				"CreateReservation_NewCustomer_Business_PreReservation");
	}
