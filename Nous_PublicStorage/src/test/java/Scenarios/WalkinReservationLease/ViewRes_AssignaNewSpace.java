...
public class ViewRes_AssignaNewSpace extends Browser_Factory {
	...
	public void afterMethod() {
		...
		if (resultFlag.equals("pass")) {
			Excel.setCellValBasedOnTcname(path, "WalkinReservationLease", "ViewRes_AssignaNewSpace", "Status", "Pass");

		} else if (resultFlag.equals("fail")) {

			Excel.setCellValBasedOnTcname(path, "WalkinReservationLease", "ViewRes_AssignaNewSpace", "Status", "Fail");
		} else {
			Excel.setCellValBasedOnTcname(path, "WalkinReservationLease", "ViewRes_AssignaNewSpace", "Status", "Skip");
		}
	...
	public Object[][] getViewReservationData() {
		return Excel.getCellValue_inlist(path, "WalkinReservationLease", "WalkinReservationLease",
				"ViewRes_AssignaNewSpace");
	}

	@Test(dataProvider = "getViewReservationData")
	public void ViewReservationAssignSpace(Hashtable<String, String> tabledata) throws InterruptedException {

		try {

			logger = extent.startTest("ViewRes_AssignaNewSpace", "View Reservation Assign A New Space");
			...
			logger.log(LogStatus.INFO, "View Reservation object created");
			...
			List<WebElement> monthlyRent = driver
					.findElements(By.xpath("//td[@class='monthly-rent-with-sales-channel']/div/div[@class='bold']"));
			List<WebElement> spaceSize = driver.findElements(By.xpath("//div[@class='size']"));

			String expMonthlyRentBeforeSwappingSpace = monthlyRent.get(0).getText();
			String expSpaceSizeBeforeSwappingSpace = spaceSize.get(0).getText().trim();
			Thread.sleep(4000);

			screen = Generic_Class.takeScreenShotPath();
			im = logger.addScreenCapture(screen);
			logger.log(LogStatus.PASS, "Captured Rent & Size Before Values successfully", im);
			logger.log(LogStatus.PASS,
					"Expected Monthly Rent is Before Swapping is:---->  " + expMonthlyRentBeforeSwappingSpace);
			logger.log(LogStatus.PASS, "Space Size Before Swapping is:---->  " + expSpaceSizeBeforeSwappingSpace);
			...
			logger.log(LogStatus.PASS, "Captured Rent & Size Before Values successfully", im);
			logger.log(LogStatus.PASS,
					"Expected Monthly Rent is After Swapping is:---->  " + actMonthlyRentAfterSwappingSpace);
			logger.log(LogStatus.PASS,
					"Expected Monthly Rent is After Swapping is:---->  " + actSpaceSizeAfterSwappingSpace);
			...
			if (expSpaceSizeBeforeSwappingSpace.equalsIgnoreCase(actualsize[0])) {
				String scpath = Generic_Class.takeScreenShotPath();
				Reporter.log(scpath, true);
				String image = logger.addScreenCapture(scpath);
			...
			((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView();",
					viewReserv.cancelReservationCancelBtn());
			Thread.sleep(3000);
