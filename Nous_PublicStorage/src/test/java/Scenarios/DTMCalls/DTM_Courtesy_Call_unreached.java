...
public class DTM_Courtesy_Call_unreached extends Browser_Factory {
	...
	public void DTM_courtesy_Call_unreached(Hashtable<String, String> tabledata) throws Exception {
		...
		try {
			...
			logger = extent.startTest("'DTM_Courtesy_Call_unreached", "'DTM_Courtesy_Call_unreached");
			...
			if(callname.isDisplayed()){
				...
				logger.log(LogStatus.PASS, "The Courtesy call is displayed");
			}else{
				
				resultFlag="Fail";
				logger.log(LogStatus.FAIL, "Courtesy call is not scheduled for the day");
			}
			Thread.sleep(2000);
			callname.click();
				...
				Thread.sleep(10000);
				 ...
				 List<WebElement> Listspacenumaftercomplition= dtmcalldetails.numberofrows_spacenumber(driver);
					...
					if (complition_call_lien.equals("1")){
						logger.log(LogStatus.PASS, "Courtesy Call for Unreached is completed value set in DB as--"+complition_call_lien);
						...
						String scpath = Generic_Class.takeScreenShotPath();
						String image = logger.addScreenCapture(scpath);
						logger.log(LogStatus.FAIL, "Courtesy Call for Unreached not  completed value sety in DB is---"+complition_call_lien);
						logger.log(LogStatus.INFO, "Courtesy Call for Unreached not completed", image);
					}	
