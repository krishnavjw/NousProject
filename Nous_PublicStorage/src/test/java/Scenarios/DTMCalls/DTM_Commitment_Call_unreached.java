...
public class DTM_Commitment_Call_unreached extends Browser_Factory {
	...
	public void DTM_commitment_Call_unreached(Hashtable<String, String> tabledata) throws Exception {
		...
		try {
			...
			logger = extent.startTest("'DTM_commitment_Call_unreached", "'DTM_commitment_Call_unreached");
			...
			String communicationcall_count=dtmdashbrd.Callcount_displaycall();
			
			int communicationcall_countconcert=Integer.parseInt(communicationcall_count);
			
			if(callname.isDisplayed() && communicationcall_countconcert>0){
				
				logger.log(LogStatus.PASS, "The Communication call is displayed with Total # Queued---"+communicationcall_countconcert);
			}else{
				
				resultFlag="Fail";
				logger.log(LogStatus.FAIL, "The Communication call is not displayed with Total # Queued---"+communicationcall_countconcert);
			}
			Thread.sleep(2000);
			callname.click();
			Thread.sleep(6000);
				...
				Thread.sleep(10000);
				 ...
				 List<WebElement> Listspacenumaftercomplition= dtmcalldetails.numberofrows_spacenumber(driver);
					...
					if (complition_call_lien.equals("1")){
						logger.log(LogStatus.PASS, "Commitment Call for Unreached is completed value set in DB as--"+complition_call_lien);
						...
						String scpath = Generic_Class.takeScreenShotPath();
						String image = logger.addScreenCapture(scpath);
						logger.log(LogStatus.FAIL, "Commitment Call for Unreached not  completed value sety in DB is---"+complition_call_lien);
						logger.log(LogStatus.INFO, "Commitment Call for Unreached not completed", image);
					}	
