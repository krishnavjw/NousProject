...
public class DTM_Courtesy_Cal_reached extends Browser_Factory {
	...
	public void DTM_Courtesy_Cal_reached_method(Hashtable<String, String> tabledata) throws Exception {
		...
		try {
			...
			logger = extent.startTest("'DTM_Courtesy_Cal_reached", "'DTM_Courtesy_Cal_reached");
			...
			String courtesycall_count=dtmdashbrd.Callcount_displaycall();
			
			int courtesycall_countconcert=Integer.parseInt(courtesycall_count);
			
			if(callname.isDisplayed() && courtesycall_countconcert>0){
				
				logger.log(LogStatus.PASS, "The Courtesy call is displayed with Total # Queued---"+courtesycall_countconcert);
			}else{
				
				resultFlag="Fail";
				logger.log(LogStatus.FAIL, "The Courtesy call is not displayed with Total # Queued---"+courtesycall_countconcert);
			}
			Thread.sleep(2000);
			callname.click();
			Thread.sleep(6000);
			...
			Thread.sleep(2000);
			 ...
			 Thread.sleep(2000);
			  List<WebElement> ListWbElebeforename=dtmcalldetails.numberofrows_customerdata(driver);
				
			  String[] arrayname = new String[ListWbElebeforename.size()];
			  
			  	
				  for (int k=0;k<ListWbElebeforename.size();k++){
					  
					  arrayname[k]= ListWbElebeforename.get(k).getText();
					  System.out.println("inserting into array---"+arrayname[k]);
					  Thread.sleep(1000);
				  }
			  Thread.sleep(2000);			
			  ...
			  Thread.sleep(5000);
			  List<WebElement> ListWbEleaftername=dtmcalldetails.numberofrows_customerdata(driver);
			 
			  String[] arrayname2 = new String[ListWbEleaftername.size()];
			  
			  	
			  for (int k=0;k<ListWbEleaftername.size();k++){
				  
				  arrayname2[k]= ListWbEleaftername.get(k).getText();
				  System.out.println("inserting into array---"+arrayname2[k]);
				  Thread.sleep(1000);
			  }
			  
			  if(ListWbElebeforename.size()>7){
				  for (int i=0,j=ListWbEleaftername.size()-1;i<5;i++,j--){
						
							  System.out.println("--actual data------- displayed---"+arrayname[i]+"----actual--"+arrayname2[j]);
						
							 
						  if(arrayname2[j].contains(arrayname[i])){
							  String scpath = Generic_Class.takeScreenShotPath();
								String image = logger.addScreenCapture(scpath);
							  
								logger.log(LogStatus.PASS, " Customer name  is sorted---"+ListWbEleaftername.get(j).getText().toString()+"--actual data displayed---"+arrayname[i]);
								  logger.log(LogStatus.INFO, " Customer name  is sorted", image);
							  }else{
								  ...
								  if(resultFlag.equals("pass"))
									...
									String scpath = Generic_Class.takeScreenShotPath();
									String image = logger.addScreenCapture(scpath);
									logger.log(LogStatus.FAIL, "Customer name  is not sorted--"+ListWbEleaftername.get(j).getText().toString()+"--actual data displayed---"+arrayname[i]);
									logger.log(LogStatus.INFO, " Customer name  is not sorted", image);
									break;
							  }
				...
				//  }
				  ...
				  for (int i=0,j=ListWbEleaftername.size()-1;i<ListWbEleaftername.size();i++,j--){
				 ...
				 Thread.sleep(2000);
				  ...
				  String[] arraypostdue = new String[ListWbElebedue.size()];
					  ...
					  for (int k=0;k<ListWbElebedue.size();k++){
						  ...
						  System.out.println("inserting into array---"+arraypostdue[k]);
						  Thread.sleep(1000);
					  }
				  Thread.sleep(2000);			
				  ...
				  for (int k=0;k<ListWbEleafterdue.size();k++){
					  ...
					  System.out.println("inserting into array---"+arraypostdue2[k]);
					  Thread.sleep(1000);
				  }
				  ...
				  if(ListWbElebedue.size()>7){
					  for (int i=0,j=ListWbEleafterdue.size()-1;i<5;i++,j--){
								  ...
								  System.out.println("--actual data------- displayed---"+arraypostdue[i]+"----actual--"+arraypostdue2[j]);
			...
			//======================= Click on any record in the list=======================================
			 ...
			 DTMCallscreen dtmcall= new DTMCallscreen(driver);
				...
				if (lastdaysdiff.equals(subdayslate[1])){
					logger.log(LogStatus.PASS, "days lete are matching---"+lastdaysdiff+"actual days left is UI--"+subdayslate[1]);
					...
					String scpath = Generic_Class.takeScreenShotPath();
					String image = logger.addScreenCapture(scpath);
					logger.log(LogStatus.FAIL, "days lete not matching--"+lastdaysdiff+"actual days left is UI--"+subdayslate[1]);
					logger.log(LogStatus.INFO, " days lete not matching", image);
				}
				Thread.sleep(2000);
				 ...
				 List<WebElement> Listspacenumaftercomplition= dtmcalldetails.numberofrows_spacenumber(driver);
					  ...
					  for (int i=0;i<5;i++){
						  ...
						  if(Listspacenumaftercomplition.get(i).getText().equals(spacenumber)){
							  if(resultFlag.equals("pass"))
									resultFlag="fail";
								logger.log(LogStatus.FAIL, "The Complited call is not removed from Courtesy List");
							 
							  
						  }else{
							  String scpath = Generic_Class.takeScreenShotPath();
								String image = logger.addScreenCapture(scpath);
							  logger.log(LogStatus.PASS, "The Complited call is  removed from Courtesy List", image);
						...
						logger.log(LogStatus.INFO, "The Complited call is not removed from Courtesy List",image);
				  }
					...
					if (complition_call_Final.equals("1")){
						logger.log(LogStatus.PASS, "Courtesy Call is completed ");
						...
						String scpath = Generic_Class.takeScreenShotPath();
						String image = logger.addScreenCapture(scpath);
						logger.log(LogStatus.FAIL, "Courtesy Call is not  completed");
						logger.log(LogStatus.INFO, "Courtesy Call is not completed", image);
					}	
					
					
					if (callreason_verify.equals("Reached")){
						logger.log(LogStatus.PASS, "Courtesy Call is completed reason--- "+callreason_verify);
						...
						String scpath = Generic_Class.takeScreenShotPath();
						String image = logger.addScreenCapture(scpath);
						logger.log(LogStatus.FAIL, "Courtesy Call is completed reason--- "+callreason_verify);
						logger.log(LogStatus.INFO, "Courtesy Call is completed reason", image);
					}	
	...
	public void afterMethod() {
		...
		if (resultFlag.equals("pass")) {
			Excel.setCellValBasedOnTcname(path, "DTMCalls", "DTM_Courtesy_Call_reached", "Status",
					"Pass");
