...
public class DTM_Courtesy_Cal_leftmsg extends Browser_Factory {
	...
	public void DTM_Courtesy_Cal_leftmsg_method(Hashtable<String, String> tabledata) throws Exception {
		...
		try {
			...
			logger = extent.startTest("'DTM_courtesy_Call_leftmsg", "'DTM_courtesy_Call_leftmsg");
					...
					Thread.sleep(10000);
					 ...
					 List<WebElement> Listspacenumaftercomplition= dtmcalldetails.numberofrows_spacenumber(driver);
						  ...
						  for (int i=0;i<5;i++){
							  ...
							  if(Listspacenumaftercomplition.get(i).getText().equals(spacenumber)){
								  if(resultFlag.equals("pass"))
									...
									String scpath = Generic_Class.takeScreenShotPath();
									String image = logger.addScreenCapture(scpath);
									logger.log(LogStatus.FAIL, "The Complited call is not removed from Courtesy call  List");
									logger.log(LogStatus.INFO, "The Complited call is not removed from Courtesy call List", image);
								 
								  
							  }else{
								  String scpath = Generic_Class.takeScreenShotPath();
									String image = logger.addScreenCapture(scpath);
								  logger.log(LogStatus.PASS, "The Complited call is  removed from Courtesy call List", image);
						...
						if (leftmessage1.equals("1")){
							logger.log(LogStatus.PASS, "Courtesy call Call left message  set in DB as--"+leftmessage1);
							...
							String scpath = Generic_Class.takeScreenShotPath();
							String image = logger.addScreenCapture(scpath);
							logger.log(LogStatus.FAIL, "Courtesy  Call left message not set in DB as--"+leftmessage1);
							logger.log(LogStatus.INFO, "Courtesy  Call left message  not set in DB as--", image);
						}	
...
//					
	...
	public void afterMethod() {
		...
		if (resultFlag.equalsIgnoreCase("pass")) {
			Excel.setCellValBasedOnTcname(path, "DTMCalls", "DTM_courtesy_Call_leftmsg", "Status",
					"Pass");
