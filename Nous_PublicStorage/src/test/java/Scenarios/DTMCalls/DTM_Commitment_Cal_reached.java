...
public class DTM_Commitment_Cal_reached extends Browser_Factory {
	...
	public void DTM_Commitment_Call_reached(Hashtable<String, String> tabledata) throws Exception {
		...
		try {
			...
			logger = extent.startTest("'DTM_Commitment_Call_reached", "'DTM_Commitment_Call_reached");
...
dtmcall.clk_Commitmentdate();
				 ...
				 List<WebElement> Listspacenumaftercomplition= dtmcalldetails.numberofrows_spacenumber(driver);
					  ...
					  for (int i=0;i<5;i++){
						  ...
						  if(Listspacenumaftercomplition.get(i).getText().equals(spacenumber)){
							  if(resultFlag.equals("pass"))
									resultFlag="fail";
								logger.log(LogStatus.FAIL, "The Complited call is not removed from Communication call List");
							 
							  
						  }else{
							  String scpath = Generic_Class.takeScreenShotPath();
								String image = logger.addScreenCapture(scpath);
							  logger.log(LogStatus.PASS, "The Complited call is  removed from Communication call List", image);
						...
						logger.log(LogStatus.INFO, "The Complited call is not removed from Communication call List",image);
				  }
					...
					if (complition_call_Final.equals("1")){
						logger.log(LogStatus.PASS, "Communication Call is completed ");
						...
						String scpath = Generic_Class.takeScreenShotPath();
						String image = logger.addScreenCapture(scpath);
						logger.log(LogStatus.FAIL, "Communication Call is not  completed");
						logger.log(LogStatus.INFO, "Communication Call is not completed", image);
					}	
					
					
					if (callreason_verify.equals("Reached")){
						logger.log(LogStatus.PASS, "Communication Call is completed reason--- "+callreason_verify);
						...
						String scpath = Generic_Class.takeScreenShotPath();
						String image = logger.addScreenCapture(scpath);
						logger.log(LogStatus.FAIL, "Communication Call is completed reason--- "+callreason_verify);
						logger.log(LogStatus.INFO, "Communication Call is completed reason", image);
					}	
	...
	public void afterMethod() {
		...
		if (resultFlag.equals("pass")) {
			Excel.setCellValBasedOnTcname(path, "DTMCalls", "DTM_Commitment_Call_reached", "Status",
					"Pass");
