...
public class CustomerSearch_SearchAllLocWithSpaceNum_StatusAll extends Browser_Factory{
	...
	{
		return Excel.getCellValue_inlist(path, "CustomerSearch","CustomerSearch","CustomerSearch_SearchAllLocWithSpaceNum_StatusAll");
	}

	@Test(dataProvider="getCustSearchData")
	public void CustomerSearch_SearchAllLocWithSpaceNum_StatusAll(Hashtable<String, String> tabledata) throws InterruptedException 
	{

		if(!(tabledata.get("Runmode").equals("Y") && tabledata.get("CustomerSearch").equals("Y")))
		{
			resultFlag="skip";
			throw new SkipException("Skipping the CustomerSearch_SearchAllLocWithSpaceNum_StatusAll Customer");
		}

		try{
			logger=extent.startTest("CustomerSearch_SearchAllLocWithSpaceNum_StatusAll", "customer search with space number in all locations with all option");
			Thread.sleep(5000);
			...
			Thread.sleep(4000);
			search.clearSpaceNum();
			search.enterSpaceNum(tabledata.get("SpaceNumber"));
			logger.log(LogStatus.PASS, "entered SpaceNumber sucessfully in Search for module");
			Thread.sleep(2000);
			...
			String SQLQuery = "select DISTINCT a.accountnumber, a.accountnumber, c.lastname from contact c with (nolock) "+ 
					...
					+"where 1=1"
					+" and s.isactive=1"
					+ "and ru.rentalunitnumber='"+tabledata.get("SpaceNumber")+"'  order by c.lastname";
	...
	public void afterMethod(){
		...
		if(resultFlag.equals("pass")){
			Excel.setCellValBasedOnTcname(path, "CustomerSearch","CustomerSearch_SearchAllLocWithSpaceNum_StatusAll" , "Status", "Pass");

		}else if (resultFlag.equals("fail")){

			Excel.setCellValBasedOnTcname(path, "CustomerSearch","CustomerSearch_SearchAllLocWithSpaceNum_StatusAll" , "Status", "Fail");
		}else{
			Excel.setCellValBasedOnTcname(path, "CustomerSearch","CustomerSearch_SearchAllLocWithSpaceNum_StatusAll" , "Status", "Skip");
		}
