...
public class AdvancedSearchUsingState_FirstName_All extends Browser_Factory {
	...
	{
		return Excel.getCellValue_inlist(path, "CustomerSearch","CustomerSearch","AdvancedSearchUsingState_FirstName_All");
	}
	...
	public void AdvancedSearchUsingState_FirstName(Hashtable<String, String> tabledata) throws Exception 
	{
		...
		try{
			logger=extent.startTest("AdvancedSearchUsingState_FirstName_All","Verifying the page with account number");
	       ...
	       element.click();
			...
			String queryCount = 
					" Select  count(distinct A.AccountNumber) "+
					...
					" Join Address Ad with(nolock) on Ad.AddressID = S.physicaladdressid "+
					" Where 1=1  "+
			...
			String query = 
						" Select  distinct A.AccountNumber, A.AccountNumber, c.firstname,c.lastname "+
						" from Account A  with(nolock) "+
						" Join Customer CU with(nolock) on A.CustomerID = CU.CustomerID  "+
						" Join Contact C with(nolock) on CU.ContactID = C.ContactID  "+
						" Join AccountOrder AO with(nolock) on A.AccountID = AO.AccountID  "+
			...
			Thread.sleep(8000);
				...
				if(driver.findElements(By.xpath("//div[@id='loadingResultsWarning']//a[text()='Ok']")).size()!=0){
					logger.log(LogStatus.PASS, "Warning message displayed -- There were more than 100 records found. Please refine your search!");
	                 String scpath=Generic_Class.takeScreenShotPath();
	  				String image=logger.addScreenCapture(scpath);
					...
					logger.log(LogStatus.FAIL, "Warning message not displayed -- There were more than 100 records found. Please refine your search!");
	                 String scpath=Generic_Class.takeScreenShotPath();
	  				String image=logger.addScreenCapture(scpath);
	  				logger.log(LogStatus.INFO, "Image",image);
				}
	...
	public void afterMethod(){
		...
		if(resultFlag.equals("pass")){
			Excel.setCellValBasedOnTcname(path, "CustomerSearch","AdvancedSearchUsingState_FirstName_All" , "Status", "Pass");

		}else if (resultFlag.equals("fail")){

			Excel.setCellValBasedOnTcname(path, "CustomerSearch","AdvancedSearchUsingState_FirstName_All" , "Status", "Fail");
		}else{
			Excel.setCellValBasedOnTcname(path, "CustomerSearch","AdvancedSearchUsingState_FirstName_All" , "Status", "Skip");
		}
