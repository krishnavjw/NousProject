...
public class AdvancedSearchUsingState_Email_CurrentlyRenting extends Browser_Factory {
	...
	{
		return Excel.getCellValue_inlist(path, "CustomerSearch","CustomerSearch","AdvancedSearchUsingState_Email_CurrentlyRenting");
	}
	...
	public void AdvancedSearchUsingState_FirstName(Hashtable<String, String> tabledata) throws Exception 
	{
		...
		try{
			logger=extent.startTest("AdvancedSearchUsingState_Email_CurrentlyRenting","Verifying the page with account number");
	       ...
	       element.click();
			...
			Thread.sleep(2000);
			advSearch.enterEmail(tabledata.get("Email"));
			Thread.sleep(2000);
			((JavascriptExecutor) driver).executeScript("window.scrollBy(5000,0)");
			Thread.sleep(5000);
			((JavascriptExecutor) driver).executeScript("window.scrollBy(0,3000)");
			...
			String queryCount="Select  count(distinct a.AccountNumber) "+
					 ...
					 " join Accountemail AE with(nolock) on AE.accountid=A.accountid "+
					 " Join EmailAddress EA with(nolock) on EA.Emailaddressid=AE.emailid "+
					 " Where 1=1   "+
					 " And Ad.StateCode= '"+tabledata.get("StateCode")+"' "+
					 " And S.isactive=1 "+
					 " And SOI.VacateDate is Null "+
					 " And ea.email='"+tabledata.get("Email")+"' ";


			
			
			
			String query="Select  distinct a.AccountNumber, a.AccountNumber, ea.email "+
						 " from Account A with(nolock) "+
						 " Join Customer CU with(nolock) on A.CustomerID = CU.CustomerID  "+
						 " Join Contact C with(nolock) on CU.ContactID = C.ContactID  "+
						 " Join AccountOrder AO with(nolock) on A.AccountID = AO.AccountID  "+
			...
			if(searchResultsName.contains(tabledata.get("Email"))){
				logger.log(LogStatus.PASS, "Email found in Search Query Heading. Email: "+tabledata.get("Email"));
			}else{
				logger.log(LogStatus.FAIL, "Email not found in Search Query Heading. Email: "+tabledata.get("Email"));
				resultFlag="fail";
			}


			logger.log(LogStatus.INFO, " *** Search Criteria - Email: "+tabledata.get("Email")+" *** ");
	...
	public void afterMethod(){
		...
		if(resultFlag.equals("pass")){
			Excel.setCellValBasedOnTcname(path, "CustomerSearch","AdvancedSearchUsingState_Email_CurrentlyRenting" , "Status", "Pass");

		}else if (resultFlag.equals("fail")){

			Excel.setCellValBasedOnTcname(path, "CustomerSearch","AdvancedSearchUsingState_Email_CurrentlyRenting" , "Status", "Fail");
		}else{
			Excel.setCellValBasedOnTcname(path, "CustomerSearch","AdvancedSearchUsingState_Email_CurrentlyRenting" , "Status", "Skip");
		}
