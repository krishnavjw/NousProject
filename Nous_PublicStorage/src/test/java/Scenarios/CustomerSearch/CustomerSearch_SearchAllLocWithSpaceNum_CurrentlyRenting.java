...
public class CustomerSearch_SearchAllLocWithSpaceNum_CurrentlyRenting extends Browser_Factory{
	...
	public void CustomerSearch_SearchAllLocWithSpaceNum_CurrRenting(Hashtable<String, String> tabledata) throws InterruptedException 
	{

		if(!(tabledata.get("Runmode").equals("Y") && tabledata.get("CustomerSearch").equals("Y")))
		{
			resultFlag="skip";
			throw new SkipException("Skipping the CustomerSearch_SearchAllLocWithSpaceNum_CurrRenting test");
		}

		try{
			logger=extent.startTest(this.getClass().getSimpleName(), "customer search with space number in all locations with currently renting  option");
			Thread.sleep(5000);
