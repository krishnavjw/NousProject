...
public class CustomerSearch_SearchAllLocWithPhoneNumber_FormerCust extends Browser_Factory {
	...
	{
		return Excel.getCellValue_inlist(path, "CustomerSearch","CustomerSearch","CustomerSearch_SearchAllLocWithPhoneNumber_FormerCust");
	}

	@Test(dataProvider="getCustSearchData")
	public void CustomerSearch_SearchAllLocWithPhoneNumber_FormerCust(Hashtable<String, String> tabledata) throws InterruptedException 
	{

		if(!(tabledata.get("Runmode").equals("Y") && tabledata.get("CustomerSearch").equals("Y")))
		{
			resultFlag="skip";
			throw new SkipException("Skipping the CustomerSearch_SearchAllLocWithPhoneNumber_FormerCust test");
		}

		try{

			logger=extent.startTest("CustomerSearch_SearchAllLocWithPhoneNumber_FormerCust", "customer search in all location using former  customer phone number");
			Thread.sleep(5000);
			...
			search.enter_Area(tabledata.get("AreaNumber"));
			logger.log(LogStatus.PASS, "entered phone area code sucessfully");

			Thread.sleep(2000);
			search.enter_Exchange(tabledata.get("Exchange"));
			logger.log(LogStatus.PASS, "entered phone Exchange number sucessfully");


			Thread.sleep(2000);
			search.enter_LineNum(tabledata.get("LineNum"));
			logger.log(LogStatus.PASS, "entered phone line number sucessfully");
			...
			search.clickSearchbtn();
			logger.log(LogStatus.PASS, "clicked on the seatch button sucessfully");
			...
			String SQLQuery = "select DISTINCT a.accountnumber, a.accountnumber, c.lastname from contact c with (nolock)"+ 
					...
					" where soi.vacatedate is not null "+
					" and s.isactive=1"+
					"and p.phonenumber ='"+tabledata.get("PhoneNumber")+"' order by c.lastname";
			...
			if(searchResultsName.replaceAll(" ", "").contains(tabledata.get("PhoneNumber"))){
				logger.log(LogStatus.PASS, "Validated Phone Number in advance search results grid");
			}else{
				logger.log(LogStatus.FAIL, "Validation failed for Phone Number in advance search results grid");
				resultFlag="fail";
			}
	...
	public void afterMethod(){
		...
		if(resultFlag.equals("pass")){
			Excel.setCellValBasedOnTcname(path, "CustomerSearch","CustomerSearch_SearchAllLocWithPhoneNumber_FormerCust" , "Status", "Pass");

		}else if (resultFlag.equals("fail")){

			Excel.setCellValBasedOnTcname(path, "CustomerSearch","CustomerSearch_SearchAllLocWithPhoneNumber_FormerCust" , "Status", "Fail");
		}else{
			Excel.setCellValBasedOnTcname(path, "CustomerSearch","CustomerSearch_SearchAllLocWithPhoneNumber_FormerCust" , "Status", "Skip");
		}
