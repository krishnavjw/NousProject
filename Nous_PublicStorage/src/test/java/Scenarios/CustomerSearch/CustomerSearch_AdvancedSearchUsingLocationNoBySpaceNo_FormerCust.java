...
public class CustomerSearch_AdvancedSearchUsingLocationNoBySpaceNo_FormerCust extends Browser_Factory {
	...
	{
		return Excel.getCellValue_inlist(path, "CustomerSearch","CustomerSearch","CustomerSearch_AdvancedSearchUsingLocationNoBySpaceNo_FormerCust");
	}


	@Test(dataProvider="getCustomerSearchData")	
	public void CustomerSearch_AdvancedSearchUsingLocationNoByIdentificationNo(Hashtable<String, String> tabledata) throws Exception 
	{
		...
		try{
			logger=extent.startTest("CustomerSearch_AdvancedSearchUsingLocationNoBySpaceNo_FormerCust","Verifying the page with account number");
			...
			logger.log(LogStatus.INFO, "Entered SpaceNumber");
			...
			String SQLQuery = "select DISTINCT a.accountnumber, a.accountnumber, c.lastname from contact c with (nolock) "+ 
					...
					+"where soi.vacatedate is not null and ru.rentalunitnumber='"+tabledata.get("SpaceNumber")+"' and s.sitenumber='"+location+"' order by c.lastname";
			...
			String SQLQueryCnt = "select count(DISTINCT a.accountnumber) from contact c with (nolock) " + 
					...
					+"left join phone p with (nolock) on p.phoneid=ap.phoneid "
					+"left join emailaddress ea with (nolock) on ea.emailaddressid=ae.emailid "
					+"where soi.vacatedate is not null and ru.rentalunitnumber='"+tabledata.get("SpaceNumber")+"' and s.sitenumber='"+location+"'";
	...
	public void afterMethod(){
		...
		if(resultFlag.equals("pass")){
			Excel.setCellValBasedOnTcname(path, "CustomerSearch","CustomerSearch_AdvancedSearchUsingLocationNoBySpaceNo_FormerCust" , "Status", "Pass");

		}else if (resultFlag.equals("fail")){

			Excel.setCellValBasedOnTcname(path, "CustomerSearch","CustomerSearch_AdvancedSearchUsingLocationNoBySpaceNo_FormerCust" , "Status", "Fail");
		}else{
			Excel.setCellValBasedOnTcname(path, "CustomerSearch","CustomerSearch_AdvancedSearchUsingLocationNoBySpaceNo_FormerCust" , "Status", "Skip");
		}
