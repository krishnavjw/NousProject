...
public class CustomerSearch_SearchAllLocWithEmailId_CurrentlyRenting extends Browser_Factory{
	...
	public void CustomerSearch_SearchAllLocWithEmailId_CurrRenting(Hashtable<String, String> tabledata) throws InterruptedException 
	{

		if(!(tabledata.get("Runmode").equals("Y") && tabledata.get("CustomerSearch").equals("Y")))
		{
			resultFlag="skip";
			throw new SkipException("Skipping the CustomerSearch_SearchAllLocWithEmailId_CurrRenting test");
		}

		try{
			
			logger=extent.startTest(this.getClass().getSimpleName(), "customer search in all location using currently renting customer Email ID");
			Thread.sleep(5000);
