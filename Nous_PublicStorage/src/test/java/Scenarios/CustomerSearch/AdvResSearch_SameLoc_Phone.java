...
public class AdvResSearch_SameLoc_Phone extends Browser_Factory{
	...
	{
		return Excel.getCellValue_inlist(path, "CustomerSearch","CustomerSearch",  "AdvResSearch_SameLoc_Phone");
	}
	

	@Test(dataProvider="getCustSearchData")
	public void advResSearch_SameLoc_Phone(Hashtable<String, String> tabledata) throws InterruptedException 
	{
		...
		try{
			...
			logger=extent.startTest("AdvResSearch_SameLoc_Phone","Advance Search for a reservation using Phone - Same Location");
			...
			//Enter phone number of a customer and search for all matching reservations
			logger.log(LogStatus.INFO, "Phone Number Search for a valid reservation");
			AdvSearch_Reservation advRes = new AdvSearch_Reservation(driver);
			advRes.clearenter_Area();
			advRes.clearenter_Exc();
			advRes.clearenter_loc();
			advRes.enter_Area(tabledata.get("AreaNumber"));
			Thread.sleep(1000);
			advRes.enter_Exc(tabledata.get("Exchange"));
			Thread.sleep(1000);
			advRes.enter_loc(tabledata.get("Location"));
			Thread.sleep(2000);
			...
			/*String sqlQuery = "select top 1 r.lastname, r.firstname, p.phonenumber, e.email, r.reservationid"+
							...
							" where p.phonenumber = '"+tabledata.get("PhoneNumber")+"' and"+
			...
			String sqlQuery = "select top 1 r.lastname, r.firstname, p.phonenumber, ea.email, r.reservationid"+
				     ...
				     " and datediff (dd, r.expirationdate,getutcdate() ) <= 30"+
				     " and R.firstname like '"+tabledata.get("FirstName")+"%'  ";
			...
			sqlQuery = "select  count(*) "+
					   ...
					   " join phone p on p.phoneid = r.phoneid "+
					   " join reservationitem ri on r.reservationid = ri.reservationid"+
					   " left join emailaddress e on e.emailaddressid = r.emailaddressid"+
					   " left join storageorderitem soi on soi.reservationitemid = ri.reservationitemid"+
					   " where p.phonenumber = '"+tabledata.get("PhoneNumber")+"' and "+
					   " r.siteid = (select siteid from site where sitenumber = "+location+")"+
					   " and t.name<> 'rented'"+
			...
			//Verify the results displayed for an invalid customer phone number
			logger.log(LogStatus.INFO, "Reservation Search for an Invalid Phone Number");
			advRes.click_Reset();
			Thread.sleep(15000);
			advRes.clearenter_Area();
			advRes.clearenter_Exc();
			advRes.clearenter_loc();
			advRes.enter_Area(tabledata.get("InvalidArea"));
			Thread.sleep(1000);
			advRes.enter_Exc(tabledata.get("InvalidExch"));
			Thread.sleep(1000);
			advRes.enter_loc(tabledata.get("InvalidLoc"));
			Thread.sleep(2000);
			((JavascriptExecutor) driver).executeScript("window.scrollBy(5000,0)");
			Thread.sleep(5000);
			((JavascriptExecutor) driver).executeScript("window.scrollBy(0,3000)");
			...
			logger.log(LogStatus.INFO, "Blank search for a customer phone number and verify the message");
			advRes.click_Reset();
			Thread.sleep(15000);
			((JavascriptExecutor) driver).executeScript("window.scrollBy(5000,0)");
			Thread.sleep(5000);
	...
	public void afterMethod(){
		...
		if(resultFlag.equals("pass")){
			Excel.setCellValBasedOnTcname(path, "CustomerSearch","AdvResSearch_SameLoc_Phone" , "Status", "Pass");

		}else if (resultFlag.equals("fail")){

			Excel.setCellValBasedOnTcname(path, "CustomerSearch","AdvResSearch_SameLoc_Phone" , "Status", "Fail");
		}else{
			Excel.setCellValBasedOnTcname(path, "CustomerSearch","AdvResSearch_SameLoc_Phone" , "Status", "Skip");
		}
