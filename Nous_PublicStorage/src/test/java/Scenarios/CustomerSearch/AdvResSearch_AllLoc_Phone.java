...
public class AdvResSearch_AllLoc_Phone extends Browser_Factory {
	...
	public void AdvanceSearchReservation_AllLoc_Phone(Hashtable<String, String> tabledata) throws InterruptedException 
	{
		...
		try{
			...
			logger=extent.startTest(this.getClass().getSimpleName(),"Advance Search for a reservation using Phone - Same Location");
			...
			/*String sqlQuery = "select top 1 r.lastname, r.firstname, p.phonenumber, e.email, r.reservationid"+
							...
							" where p.phonenumber = '"+tabledata.get("PhoneNumber")+"'"+
							" and t.name<> 'rented'"+
			...
			String sqlQuery = "select top 1 r.lastname, r.firstname, p.phonenumber, ea.email, r.reservationid"+
				     ...
				     " and s.isactive=1"+
				     " and datediff (dd, r.expirationdate,getutcdate() ) <= 30"+
				     " p.phonenumber = '"+tabledata.get("PhoneNumber")+"'";
