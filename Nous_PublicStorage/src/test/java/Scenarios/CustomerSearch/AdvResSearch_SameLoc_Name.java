...
public class AdvResSearch_SameLoc_Name extends Browser_Factory{
	...
	{
		return Excel.getCellValue_inlist(path, "CustomerSearch","CustomerSearch",  "AdvResSearch_SameLoc_Name");
	}
	

	@Test(dataProvider="getCustSearchData")
	public void advResSearch_SameLoc_Name(Hashtable<String, String> tabledata) throws InterruptedException 
	{
		...
		try{
			...
			logger=extent.startTest("AdvResSearch_SameLoc_Name","Advance Search for a reservation using Name - Same Location");
			...
			//Enter name of a customer and search for all matching reservations
			logger.log(LogStatus.INFO, "Name Search for a valid reservation");
			AdvSearch_Reservation advRes = new AdvSearch_Reservation(driver);
			advRes.enter_FirstName(tabledata.get("FirstName"));
			Thread.sleep(2000);
			...
			/*String sqlQuery = "select top 1 r.lastname, r.firstname, p.phonenumber, e.email, r.reservationid"+
							...
							" where r.firstname like '"+tabledata.get("FirstName")+"%' and"+
			...
			//Verify the results displayed for an invalid customer name
			logger.log(LogStatus.INFO, "Reservation Search for an Invalid Name");
			advRes.click_Reset();
			Thread.sleep(15000);
			advRes.enter_FirstName(tabledata.get("NOnExCustomer"));
			Thread.sleep(5000);
			((JavascriptExecutor) driver).executeScript("window.scrollBy(5000,0)");
			Thread.sleep(5000);
			((JavascriptExecutor) driver).executeScript("window.scrollBy(0,3000)");
			...
			logger.log(LogStatus.INFO, "Blank search for a customer name and verify the message");
			advRes.click_Reset();
			Thread.sleep(15000);
			((JavascriptExecutor) driver).executeScript("window.scrollBy(5000,0)");
			Thread.sleep(5000);
	...
	public void afterMethod(){
		...
		if(resultFlag.equals("pass")){
			Excel.setCellValBasedOnTcname(path, "CustomerSearch","AdvResSearch_SameLoc_Name" , "Status", "Pass");

		}else if (resultFlag.equals("fail")){

			Excel.setCellValBasedOnTcname(path, "CustomerSearch","AdvResSearch_SameLoc_Name" , "Status", "Fail");
		}else{
			Excel.setCellValBasedOnTcname(path, "CustomerSearch","AdvResSearch_SameLoc_Name" , "Status", "Skip");
		}
