...
public class CustomerSearch_AdvancedSearchUsingLocationNoByCompanyName_CurrentlyRenting extends Browser_Factory {
	...
	public void CustomerSearch_AdvancedSearchUsingLocationNoByCompanyName_CurrentlyRenting(Hashtable<String, String> tabledata) throws Exception 
	{
		...
		try{
		    logger=extent.startTest(this.getClass().getSimpleName(),"Advance Search using Location Num by Company Name - Currently Renting");
		   ...
		   ((JavascriptExecutor)driver).executeScript("window.scrollTo(500, 0)");
			...
			String SQLQueryCnt = "select count(DISTINCT a.AccountNumber) from contact c with (nolock) "+
					...
					"and cu.CompanyName like '"+tabledata.get("CompanyName")+"%' and s.sitenumber='"+location+"'";
			...
			String SQLQuery = "select DISTINCT a.AccountNumber from contact c with (nolock) "+
					...
					"left join emailaddress ea with (nolock) on ea.emailaddressid=ae.emailid "+
					" where soi.vacatedate is null "+
					"and cu.CompanyName like '"+tabledata.get("CompanyName")+"%' and s.sitenumber='"+location+"' order by a.AccountNumber ASC";
