...
public class CustomerSearch_SearchAllLocWithLastName_StatusAll extends Browser_Factory {
	...
	{
		return Excel.getCellValue_inlist(path, "CustomerSearch","CustomerSearch","CustomerSearch_SearchAllLocWithLastName_StatusAll");
	}

	@Test(dataProvider="getCustSearchData")
	public void CustomerSearch_SearchAllLocWithLastName_StatusAll(Hashtable<String, String> tabledata) throws InterruptedException 
	{

		if(!(tabledata.get("Runmode").equals("Y") && tabledata.get("CustomerSearch").equals("Y")))
		{
			resultFlag="skip";
			throw new SkipException("Skipping the CustomerSearch_SearchAllLocWithLastName_StatusAll test");
		}

		try{

			logger=extent.startTest("CustomerSearch_SearchAllLocWithLastName_StatusAll", "customer search in all location using customer last name");
			Thread.sleep(5000);
			...
			Thread.sleep(4000);
			search.enterLastName(tabledata.get("LastName"));
			logger.log(LogStatus.PASS, "entered customer Last name sucessfully in Search for module");
			...
			String SQLQuery = "select  DISTINCT a.accountnumber, a.accountnumber, c.lastname from contact c with (nolock) "+ 
					...
					"where 1=1"+
					" and s.isactive=1"+
					" and c.lastname like '"+tabledata.get("LastName")+"%'  order by c.lastname";
			...
			String SQLQueryCnt = "select count(DISTINCT a.accountnumber) from contact c with (nolock) " + 
					...
					"where 1=1"+
					" and s.isactive=1"+
					" and c.lastname like '"+tabledata.get("LastName")+"%'";
			...
			if(Integer.parseInt(ele2)<100){
				...
				if(text.contains("Warning: There were more than 100 records found. Please refine your search")){
					...
					logger.log(LogStatus.FAIL, "More than 100 results found in db. But, warning message not displayed in UI. DB Count: "+ele2);
					String scpath=Generic_Class.takeScreenShotPath();
					String image=logger.addScreenCapture(scpath);
					logger.log(LogStatus.INFO, "Image",image);
				}
			...
			if(searchResultsName.contains(tabledata.get("LastName"))){
				logger.log(LogStatus.PASS, "Validated Last Name in advance search results grid");
			}else{
				logger.log(LogStatus.FAIL, "Validation failed for Last Name in advance search results grid");
				resultFlag="fail";
			}
	...
	public void afterMethod(){
		...
		if(resultFlag.equals("pass")){
			Excel.setCellValBasedOnTcname(path, "CustomerSearch","CustomerSearch_SearchAllLocWithLastName_StatusAll" , "Status", "Pass");

		}else if (resultFlag.equals("fail")){

			Excel.setCellValBasedOnTcname(path, "CustomerSearch","CustomerSearch_SearchAllLocWithLastName_StatusAll" , "Status", "Fail");
		}else{
			Excel.setCellValBasedOnTcname(path, "CustomerSearch","CustomerSearch_SearchAllLocWithLastName_StatusAll" , "Status", "Skip");
		}
