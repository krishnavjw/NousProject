...
public class AdvancedSearchUsingState_LastName_FormerCustomer extends Browser_Factory {
	...
	{
		return Excel.getCellValue_inlist(path, "CustomerSearch","CustomerSearch","AdvancedSearchUsingState_LastName_FormerCustomer");
	}
	...
	public void AdvancedSearchUsingState_FirstName(Hashtable<String, String> tabledata) throws Exception 
	{
		...
		logger=extent.startTest("AdvancedSearchUsingState_LastName_FormerCustomer","Verifying the page with account number");
		...
		try{
	       ...
	       element.click();
			...
			String query="Select  distinct A.AccountNumber, A.AccountNumber, c.firstname,c.lastname "+
					"from Account A "+ 
					"Join Customer CU on A.CustomerID = CU.CustomerID "+
					"Join Contact C on CU.ContactID = C.ContactID "+
					"Join AccountOrder AO on A.AccountID = AO.AccountID "+
			...
			boolean noResults = false;
			 ...
			 String scpath=Generic_Class.takeScreenShotPath();
				...
				if(searchResultsName.contains(tabledata.get("LastName"))){
					logger.log(LogStatus.PASS, "Last Name found in Search Query Heading. Last Name: "+tabledata.get("LastName"));
				}else{
					logger.log(LogStatus.FAIL, "Last Name not found in Search Query Heading. Last Name: "+tabledata.get("LastName"));
					resultFlag="fail";
				}

				
				logger.log(LogStatus.INFO, " *** Search Criteria - Last Name: "+tabledata.get("LastName")+" *** ");
	...
	public void afterMethod(){
		...
		if(resultFlag.equals("pass")){
			Excel.setCellValBasedOnTcname(path, "CustomerSearch","AdvancedSearchUsingState_LastName_FormerCustomer" , "Status", "Pass");

		}else if (resultFlag.equals("fail")){

			Excel.setCellValBasedOnTcname(path, "CustomerSearch","AdvancedSearchUsingState_LastName_FormerCustomer" , "Status", "Fail");
		}else{
			Excel.setCellValBasedOnTcname(path, "CustomerSearch","AdvancedSearchUsingState_LastName_FormerCustomer" , "Status", "Skip");
		}
