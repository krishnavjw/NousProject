...
public class CustomerSearch_SearchAllLocWithCompanyName_StatusAll extends Browser_Factory{
	...
	{
		return Excel.getCellValue_inlist(path, "CustomerSearch","CustomerSearch","CustomerSearch_SearchAllLocWithCompanyName_StatusAll");
	}

	@Test(dataProvider="getCustSearchData")
	public void CustomerSearch_SearchAllLocWithCompanyName_StatusAll(Hashtable<String, String> tabledata) throws InterruptedException 
	{

		if(!(tabledata.get("Runmode").equals("Y") && tabledata.get("CustomerSearch").equals("Y")))
		{
			resultFlag="skip";
			throw new SkipException("Skipping the CustomerSearch_SearchAllLocWithCompanyName_StatusAll test");
		}

		try{

			logger=extent.startTest("CustomerSearch_SearchAllLocWithCompanyName_StatusAll", "customer search in all location using customer company name ");
			Thread.sleep(5000);
			...
			Thread.sleep(4000);
			search.enterCmpName(tabledata.get("CompanyName"));
			logger.log(LogStatus.PASS, "entered customer company name sucessfully in Search for module");
            ...
            if(searchResultsName.contains(tabledata.get("CompanyName"))){
                  logger.log(LogStatus.PASS, "Validated Company Name in advance search results grid");
            }else{
            	 logger.log(LogStatus.FAIL, "Validation failed for Company Name in advance search results grid");
            	 resultFlag="fail";
            }

            
			String SQLQuery = "select DISTINCT a.AccountNumber from contact c with (nolock) "+
					"join customer cu with (nolock) on cu.contactid=c.contactid join account a on a.customerid=cu.customerid "+ 
					"join accountorder ao with (nolock) on ao.accountid=a.accountid "+
					"join accountorderitem aoi with (nolock) on aoi.accountorderid=ao.accountorderid "+
					"join storageorderitem soi with (nolock) on soi.storageorderitemid=aoi.storageorderitemid "+
					...
					"where 1=1 "+
					" and s.isactive=1"+
					" and cu.CompanyName like '"+tabledata.get("CompanyName")+"%'  order by a.AccountNumber ASC";
			
			
			ArrayList<String> dbAccNums = DataBase_JDBC.executeSQLQuery_List(SQLQuery);
	...
	public void afterMethod(){
		...
		if(resultFlag.equals("pass")){
			Excel.setCellValBasedOnTcname(path, "CustomerSearch","CustomerSearch_SearchAllLocWithCompanyName_StatusAll" , "Status", "Pass");

		}else if (resultFlag.equals("fail")){

			Excel.setCellValBasedOnTcname(path, "CustomerSearch","CustomerSearch_SearchAllLocWithCompanyName_StatusAll" , "Status", "Fail");
		}else{
			Excel.setCellValBasedOnTcname(path, "CustomerSearch","CustomerSearch_SearchAllLocWithCompanyName_StatusAll" , "Status", "Skip");
		}
