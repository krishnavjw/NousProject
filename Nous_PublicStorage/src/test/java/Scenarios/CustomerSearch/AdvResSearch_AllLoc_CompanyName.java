...
public class AdvResSearch_AllLoc_CompanyName extends Browser_Factory {
	...
	public void advResSearch_SameLoc_CompanyName(Hashtable<String, String> tabledata) throws InterruptedException 
	{
		...
		try{
			...
			logger=extent.startTest(this.getClass().getSimpleName(),"Advance Search for a reservation using Company Name - Same Location");
			...
			//Enter company name of a customer and search for all matching reservations
			logger.log(LogStatus.INFO, "Company Name Search for a valid reservation");
			AdvSearch_Reservation advRes = new AdvSearch_Reservation(driver);
			advRes.enter_CmpnyName(tabledata.get("CompanyName"));
			
			//Check SearhAll Check box
			advRes.click_SearchAllLocChkbox();
			logger.log(LogStatus.PASS, "Clicked on Search All Location Check box ");
			...
			String sqlQuery = "select top 1 r.lastname, r.firstname, p.phonenumber, ea.email, r.reservationid"+
					...
					" join account A with (nolock) on A.Accountid=R.reservationaccountid"+
					...
					" left join rentalunit ru with (nolock) on ru.rentalunitid=ri.itemid and ri.itemtypeid=132"+
					" where cu.customertypeid=91"+
					" And S.isactive=1 "+
					" and t.name<> 'rented'"+
					" and datediff (dd, r.expirationdate,getutcdate() ) <= 30"+
					" and cu.companyname like '"+tabledata.get("CompanyName")+"%' order by r.reservationid asc";
			...
			sqlQuery = "select count(*)"+
					...
					" and t.name<> 'rented'"+
					" and datediff (dd, r.expirationdate,getutcdate() ) <= 30"+
					" and cu.companyname like '"+tabledata.get("CompanyName")+"%'";
			...
			for(int i=0; i<resValues.size(); i++){
				...
				getText = driver.findElement(By.xpath("//div[@class='searchResult k-grid k-widget']/div[contains(@class,'k-grid-content ps-container')]/table/tbody/tr/td["+(i+2)+"]")).getText().trim();
				if(i==2){
			...
			//Verify the results displayed for an invalid customer company name
			logger.log(LogStatus.INFO, "Reservation Search for an Invalid Company Name");
			advRes.click_Reset();
			Thread.sleep(15000);
			advRes.enter_CmpnyName(tabledata.get("InvalidCmpny"));
			Thread.sleep(2000);
			((JavascriptExecutor) driver).executeScript("window.scrollBy(5000,0)");
			Thread.sleep(5000);
			((JavascriptExecutor) driver).executeScript("window.scrollBy(0,3000)");
			...
			((JavascriptExecutor) driver).executeScript("window.scrollBy(-2000,0)");
			String getText1 = advRes.getText_records_match().toLowerCase();
			scpath=Generic_Class.takeScreenShotPath();
			Reporter.log(scpath,true);
			image=logger.addScreenCapture(scpath);
			if(getText1.contains("no results found matching your criteria")){
			...
			//Do not enter any details for customer company name and verify the message
			logger.log(LogStatus.INFO, "Blank search for a customer company name and verify the message");
			advRes.click_Reset();
			Thread.sleep(15000);
			((JavascriptExecutor) driver).executeScript("window.scrollBy(5000,0)");
			Thread.sleep(5000);
			...
			((JavascriptExecutor) driver).executeScript("window.scrollBy(-2000,0)");
			getText1 = advRes.getText_records_match().toLowerCase();
			scpath=Generic_Class.takeScreenShotPath();
			Reporter.log(scpath,true);
			image=logger.addScreenCapture(scpath);
			if(getText1.contains("no criteria was entered")){
