...
public class CustomerSearch_SearchAllLocWithEmailId_StatusAll extends Browser_Factory {
	...
	{
		return Excel.getCellValue_inlist(path, "CustomerSearch","CustomerSearch","CustomerSearch_SearchAllLocWithEmailId_StatusAll");
	}
	
	@Test(dataProvider="getCustSearchData")
	public void CustomerSearch_SearchAllLocWithEmailId_StatusAll(Hashtable<String, String> tabledata) throws InterruptedException 
	{

		if(!(tabledata.get("Runmode").equals("Y") && tabledata.get("CustomerSearch").equals("Y")))
		{
			resultFlag="skip";
			throw new SkipException("Skipping the CustomerSearch_SearchAllLocWithEmailId_StatusAll test");
		}

		try{
			
			logger=extent.startTest("CustomerSearch_SearchAllLocWithEmailId_StatusAll", "customer search in all location using  customer Email ID");
			Thread.sleep(5000);
	...
	public void afterMethod(){
		...
		if(resultFlag.equals("pass")){
			Excel.setCellValBasedOnTcname(path, "CustomerSearch","CustomerSearch_SearchAllLocWithEmailId_StatusAll" , "Status", "Pass");

		}else if (resultFlag.equals("fail")){

			Excel.setCellValBasedOnTcname(path, "CustomerSearch","CustomerSearch_SearchAllLocWithEmailId_StatusAll" , "Status", "Fail");
		}else{
			Excel.setCellValBasedOnTcname(path, "CustomerSearch","CustomerSearch_SearchAllLocWithEmailId_StatusAll" , "Status", "Skip");
		}
