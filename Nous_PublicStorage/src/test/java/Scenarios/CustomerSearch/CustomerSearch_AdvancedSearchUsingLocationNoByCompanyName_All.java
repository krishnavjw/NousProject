...
public class CustomerSearch_AdvancedSearchUsingLocationNoByCompanyName_All extends Browser_Factory {
	...
	public void CustomerSearch_AdvancedSearchUsingLocationNoByCompanyName(Hashtable<String, String> tabledata) throws Exception 
	{
		...
		try{
			...
			String SQLQueryCnt = "select count(DISTINCT a.AccountNumber) from contact c with (nolock) "+
					...
					"where "+
					" cu.CompanyName like '"+tabledata.get("CompanyName")+"%' and s.sitenumber='"+location+"'";
			...
			String SQLQuery = "select DISTINCT a.AccountNumber from contact c with (nolock) "+
					...
					"left join emailaddress ea with (nolock) on ea.emailaddressid=ae.emailid "+
					"where  "+
					"cu.CompanyName like '"+tabledata.get("CompanyName")+"%' and s.sitenumber='"+location+"' order by a.AccountNumber ASC";
