...
public class CustomerSearch_SearchAllLocWithFirstName_StatusAll extends Browser_Factory {
	...
	{
		return Excel.getCellValue_inlist(path, "CustomerSearch","CustomerSearch","CustomerSearch_SearchAllLocWithFirstName_StatusAll");
	}

	@Test(dataProvider="getCustSearchData")
	public void CustomerSearch_SearchAllLocWithFirstName_StatusAll(Hashtable<String, String> tabledata) throws InterruptedException 
	{

		if(!(tabledata.get("Runmode").equals("Y") && tabledata.get("CustomerSearch").equals("Y")))
		{
			resultFlag="skip";
			throw new SkipException("Skipping the CustomerSearch_SearchAllLocWithFirstName_StatusAll test");
		}

		try{

			logger=extent.startTest("CustomerSearch_SearchAllLocWithFirstName_StatusAll", "customer search in all location using customer first name ");
			Thread.sleep(5000);
			...
			search.clickStatusDropdown();
			Thread.sleep(2000);
			statusTypes= driver.findElements(By.xpath("//ul[@id='SearchContract_SearchStatusID_listbox']/li[@class='k-item']"));
			for(WebElement type:statusTypes)
			{
				if(type.getText().trim().equalsIgnoreCase("All"))
				{
			...
			logger.log(LogStatus.INFO, "Selected All from status drop down");
			logger.log(LogStatus.PASS, "selecting the all  option from the dropdown");
			Thread.sleep(4000);
			search.enterFirstName(tabledata.get("FirstName"));
			...
			String SQlQuery = "select  DISTINCT a.accountnumber, a.accountnumber, c.firstname, c.lastname from contact c with (nolock) "+
					...
					"where 1=1 "+
					" and s.isactive=1"+
					" and c.firstname like '"+tabledata.get("FirstName")+"%'  order by c.lastname";
			...
			String SQLQueryCnt = "select count(DISTINCT a.accountnumber) from contact c with (nolock) " + 
					...
					"where 1=1 "+
					" and s.isactive=1"+
					" and c.firstname like '"+tabledata.get("FirstName")+"%'";
			...
			if(searchResultsName.contains(tabledata.get("FirstName"))){
				logger.log(LogStatus.PASS, "First Name found in Search Query Heading. First Name: "+tabledata.get("FirstName"));
			}else{
				logger.log(LogStatus.FAIL, "First Name not found in Search Query Heading. First Name: "+tabledata.get("FirstName"));
				resultFlag="fail";
			}


			logger.log(LogStatus.INFO, " *** Search Criteria - First Name: "+tabledata.get("FirstName")+" *** ");
	...
	public void afterMethod(){
		...
		if(resultFlag.equals("pass")){
			Excel.setCellValBasedOnTcname(path, "CustomerSearch","CustomerSearch_SearchAllLocWithFirstName_StatusAll" , "Status", "Pass");

		}else if (resultFlag.equals("fail")){

			Excel.setCellValBasedOnTcname(path, "CustomerSearch","CustomerSearch_SearchAllLocWithFirstName_StatusAll" , "Status", "Fail");
		}else{
			Excel.setCellValBasedOnTcname(path, "CustomerSearch","CustomerSearch_SearchAllLocWithFirstName_StatusAll" , "Status", "Skip");
		}
