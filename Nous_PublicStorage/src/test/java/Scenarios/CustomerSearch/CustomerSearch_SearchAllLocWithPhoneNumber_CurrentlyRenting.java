...
public class CustomerSearch_SearchAllLocWithPhoneNumber_CurrentlyRenting extends Browser_Factory{
	...
	public void CustomerSearch_SearchAllLocWithPhoneNumber_CurrRenting(Hashtable<String, String> tabledata) throws InterruptedException 
	{

		if(!(tabledata.get("Runmode").equals("Y") && tabledata.get("CustomerSearch").equals("Y")))
		{
			resultFlag="skip";
			throw new SkipException("Skipping the CustomerSearch_SearchAllLocWithPhoneNumber_CurrRenting test");
		}

		try{

			logger=extent.startTest(this.getClass().getSimpleName(), "customer search in all location using currently renting customer phone number");
			Thread.sleep(5000);
			...
			String SQLQuery = "select DISTINCT a.accountnumber, a.accountnumber, c.lastname from contact c "+ 
					...
					" where 1=1 "+
					" and s.isactive=1"+
					"and soi.vacatedate is null and p.phonenumber ='"+tabledata.get("PhoneNumber")+"'  order by c.lastname";


			String SQLQueryCnt = "select count(DISTINCT a.accountnumber) from contact c  " + 
					"join customer cu  on cu.contactid=c.contactid join account a on a.customerid=cu.customerid "+ 
					"join accountorder ao  on ao.accountid=a.accountid "+
					"join accountorderitem aoi  on aoi.accountorderid=ao.accountorderid "+
					"join storageorderitem soi  on soi.storageorderitemid=aoi.storageorderitemid "+
					...
					"where 1=1 "+
					" and s.isactive=1"+
					"and soi.vacatedate is null and p.phonenumber ='"+tabledata.get("PhoneNumber")+"'";
