...
public class BasicSearch_Email extends Browser_Factory{
	...
	{
		return Excel.getCellValue_inlist(path, "CustomerSearch","CustomerSearch","BasicSearch_Email");
	}
	...
	public void BasicSearch_Name(Hashtable<String, String> tabledata) throws InterruptedException 
	{
		...
		try{
			...
			logger=extent.startTest("BasicSearch_Email", "Search in basic search ");
			...
			//Enter "Email" of an existing customer at current location and click on "Find Customer" button

			pmhomepage.enter_findCustAddrLocation(tabledata.get("Email").toLowerCase());
			logger.log(LogStatus.PASS, " existing customer email entered successfully ");
			Thread.sleep(5000);
			...
			boolean noResults = false;
			 ...
			 if(list2.get(0).getText().equalsIgnoreCase("NO ITEMS TO DISPLAY")){
	          ...
	          /*  if(list2.size()== Integer.parseInt(ele2)){
	            ...
	            if(searchResultsName.toLowerCase().contains(tabledata.get("Email").toLowerCase())){
			...
			//Enter details " email" of a non existing customer at current location and click on "Find Customer" button and verify info message
			pmhomepage.enter_findCustAddrLocation("brajeshk@nousinfo.com");
	...
	public void afterMethod(){
		...
		if(resultFlag.equals("pass")){
			Excel.setCellValBasedOnTcname(path, "CustomerSearch","BasicSearch_Email" , "Status", "Pass");

		}else if (resultFlag.equals("fail")){

			Excel.setCellValBasedOnTcname(path, "CustomerSearch","BasicSearch_Email" , "Status", "Fail");
		}else{
			Excel.setCellValBasedOnTcname(path, "CustomerSearch","BasicSearch_Email" , "Status", "Skip");
		}
