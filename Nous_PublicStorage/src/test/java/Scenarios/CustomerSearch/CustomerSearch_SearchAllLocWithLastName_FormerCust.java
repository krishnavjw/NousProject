...
public class CustomerSearch_SearchAllLocWithLastName_FormerCust extends Browser_Factory{
	...
	{
		return Excel.getCellValue_inlist(path, "CustomerSearch","CustomerSearch","CustomerSearch_SearchAllLocWithLastName_FormerCust");
	}

	@Test(dataProvider="getCustSearchData")
	public void CustomerSearch_SearchAllLocWithLastName_FormerCust(Hashtable<String, String> tabledata) throws InterruptedException 
	{

		if(!(tabledata.get("Runmode").equals("Y") && tabledata.get("CustomerSearch").equals("Y")))
		{
			resultFlag="skip";
			throw new SkipException("Skipping the CustomerSearch_SearchAllLocWithLastName_FormerCust test");
		}

		try{

			logger=extent.startTest("CustomerSearch_SearchAllLocWithLastName_FormerCust", "customer search in all location using former customer last name");
			Thread.sleep(5000);
	...
	public void afterMethod(){
		...
		if(resultFlag.equals("pass")){
			Excel.setCellValBasedOnTcname(path, "CustomerSearch","CustomerSearch_SearchAllLocWithLastName_FormerCust" , "Status", "Pass");

		}else if (resultFlag.equals("fail")){

			Excel.setCellValBasedOnTcname(path, "CustomerSearch","CustomerSearch_SearchAllLocWithLastName_FormerCust" , "Status", "Fail");
		}else{
			Excel.setCellValBasedOnTcname(path, "CustomerSearch","CustomerSearch_SearchAllLocWithLastName_FormerCust" , "Status", "Skip");
		}
