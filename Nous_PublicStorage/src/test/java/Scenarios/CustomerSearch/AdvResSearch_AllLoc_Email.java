...
public class AdvResSearch_AllLoc_Email extends Browser_Factory {
	...
	public void AdvanceSearchReservation_AllLoc_Email(Hashtable<String, String> tabledata) throws InterruptedException 
	{
		...
		try{
			...
			logger=extent.startTest(this.getClass().getSimpleName(),"Advance Search for a reservation using Email - Same Location");
			...
			String sqlQuery = "select top 1 r.lastname, r.firstname, p.phonenumber, e.email, r.reservationid"+
					...
					" where e.email = '"+tabledata.get("EmailId")+"'"+
					" and t.name<> 'rented'"+
					" And S.isactive=1 "+
