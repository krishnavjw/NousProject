...
public class AdvancedSearchUsingState_SpaceNumber_FormerCustomer extends Browser_Factory {
	...
	{
		return Excel.getCellValue_inlist(path, "CustomerSearch","CustomerSearch","AdvancedSearchUsingState_SpaceNumber_FormerCustomer");
	}
	...
	public void AdvancedSearchUsingState_FirstName(Hashtable<String, String> tabledata) throws Exception 
	{
		...
		try{
			logger=extent.startTest("AdvancedSearchUsingState_SpaceNumber_FormerCustomer","Verifying the page with account number");
	       ...
	       element.click();
			...
			advSearch.enterSpaceNum(tabledata.get("SpaceNumber"));
			Thread.sleep(2000);
			((JavascriptExecutor) driver).executeScript("window.scrollBy(5000,0)");
			Thread.sleep(5000);
			((JavascriptExecutor) driver).executeScript("window.scrollBy(0,3000)");
			...
			String queryCount="Select count(distinct A.AccountNumber) "+
					...
					"Join RentalUnit RU on SOI.RentalUnitID = RU.RentalUnitID "+
					"Where 1=1 "+
					"And S.isactive=1 "+
					"And SOI.VacateDate is not Null "+
					"And Ad.StateCode='"+tabledata.get("StateCode")+"' "+
					"And RU.RentalUnitNumber = '"+tabledata.get("SpaceNumber")+"' ";


			String query="Select distinct A.AccountNumber, A.AccountNumber,c.firstname,ru.rentalunitnumber "+
					"from Account A "+
					"Join Customer CU on A.CustomerID = CU.CustomerID "+
					"Join Contact C on CU.ContactID = C.ContactID "+
					"Join AccountOrder AO on A.AccountID = AO.AccountID "+
			...
			if(searchResultsName.contains(tabledata.get("SpaceNumber"))){
				logger.log(LogStatus.PASS, "Space Number found in Search Query Heading. Space Number: "+tabledata.get("SpaceNumber"));
			}else{
				logger.log(LogStatus.FAIL, "Space Number not found in Search Query Heading. Space Number: "+tabledata.get("SpaceNumber"));
				resultFlag="fail";
			}


			logger.log(LogStatus.INFO, " *** Search Criteria - Space Number: "+tabledata.get("SpaceNumber")+" *** ");
	...
	public void afterMethod(){
		...
		if(resultFlag.equals("pass")){
			Excel.setCellValBasedOnTcname(path, "CustomerSearch","AdvancedSearchUsingState_SpaceNumber_FormerCustomer" , "Status", "Pass");

		}else if (resultFlag.equals("fail")){

			Excel.setCellValBasedOnTcname(path, "CustomerSearch","AdvancedSearchUsingState_SpaceNumber_FormerCustomer" , "Status", "Fail");
		}else{
			Excel.setCellValBasedOnTcname(path, "CustomerSearch","AdvancedSearchUsingState_SpaceNumber_FormerCustomer" , "Status", "Skip");
		}
