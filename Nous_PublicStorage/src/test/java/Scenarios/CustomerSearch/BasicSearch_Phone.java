...
public class BasicSearch_Phone extends Browser_Factory{
	...
	{
		return Excel.getCellValue_inlist(path, "CustomerSearch","CustomerSearch","BasicSearch_Phone");
	}
	...
	public void BasicSearch_Name(Hashtable<String, String> tabledata) throws InterruptedException 
	{
		...
		try{
			...
			logger=extent.startTest("BasicSearch_Phone", "Search in basic search ");
			...
			//Enter "Phone" of an existing customer at current location and click on "Find Customer" button
			pmhomepage.enter_findCustAddrLocation(tabledata.get("PhoneNumber").toLowerCase());
			logger.log(LogStatus.PASS, " existing customer phone number entered successfully in lower case");
			Thread.sleep(5000);
			...
			String SQLQuery = "select DISTINCT a.accountnumber, a.accountnumber, c.lastname from contact c with (nolock)"+ 
					...
					" where soi.vacatedate is null and p.phonenumber ='"+tabledata.get("PhoneNumber")+"' and s.sitenumber='"+location+"' order by c.lastname";
			...
			String SQLQueryCnt = "select count(DISTINCT a.accountnumber) from contact c with (nolock) " + 
					...
					"left join phone p with (nolock) on p.phoneid=ap.phoneid "+
					"left join emailaddress ea with (nolock) on ea.emailaddressid=ae.emailid "+
					"where soi.vacatedate is null and p.phonenumber ='"+tabledata.get("PhoneNumber")+"' and s.sitenumber='"+location+"'";
			...
			pmhomepage.enter_findCustAddrLocation("9944975057");
			logger.log(LogStatus.PASS, " Non existing customer Phone no entered successfully ");
			Thread.sleep(5000);
			pmhomepage.clk_findCustomer();
			logger.log(LogStatus.PASS, "Click on Find Customer Button successfully");
			...
			if(searchResultTxt.contains("NO RESULTS FOUND MATCHING YOUR CRITERIA")){
				...
				logger.log(LogStatus.INFO, "Info Message is  successfully verified ",image1);
			}
			else{
				...
				if(resultFlag.equals("pass"))
					resultFlag="fail";
				String image2=logger.addScreenCapture(scpath);
				logger.log(LogStatus.FAIL, "Info Message is not  verified ");
				logger.log(LogStatus.INFO, "Info Message is not   verified ",image2);
	...
	public void afterMethod(){
		...
		if(resultFlag.equals("pass")){
			Excel.setCellValBasedOnTcname(path, "CustomerSearch","BasicSearch_Phone" , "Status", "Pass");

		}else if (resultFlag.equals("fail")){

			Excel.setCellValBasedOnTcname(path, "CustomerSearch","BasicSearch_Phone" , "Status", "Fail");
		}else{
			Excel.setCellValBasedOnTcname(path, "CustomerSearch","BasicSearch_Phone" , "Status", "Skip");
		}
