...
public class CustomerSearch_AdvancedSearchUsingLocationNoByPhoneNo_FormerCust extends Browser_Factory {
	...
	{
		return Excel.getCellValue_inlist(path, "CustomerSearch","CustomerSearch","CustomerSearch_AdvancedSearchUsingLocationNoByPhoneNo_FormerCust");
	}


	@Test(dataProvider="getCustomerSearchData")	
	public void CustomerSearch_AdvancedSearchUsingLocationNoByCompanyName(Hashtable<String, String> tabledata) throws Exception 
	{
		...
		try{
			logger=extent.startTest("CustomerSearch_AdvancedSearchUsingLocationNoByPhoneNo_FormerCust","Verifying the page with account number");
			...
			logger.log(LogStatus.INFO, "Entered Area Number");
			advSearch.enter_Exchange(tabledata.get("Exchange"));
			logger.log(LogStatus.INFO, "Entered Exchange Number");
			advSearch.enter_LineNum(tabledata.get("LineNum"));
			logger.log(LogStatus.INFO, "Entered Location Number");
			...
			String SQLQuery = "select DISTINCT a.accountnumber, a.accountnumber, c.lastname from contact c with (nolock)"+ 
					...
					" where soi.vacatedate is not null and p.phonenumber ='"+tabledata.get("PhoneNumber")+"' and s.sitenumber='"+location+"' order by c.lastname";
			...
			String SQLQueryCnt = "select count(DISTINCT a.accountnumber) from contact c with (nolock) " + 
					...
					"left join phone p with (nolock) on p.phoneid=ap.phoneid "+
					"left join emailaddress ea with (nolock) on ea.emailaddressid=ae.emailid "+
					"where soi.vacatedate is not null and p.phonenumber ='"+tabledata.get("PhoneNumber")+"' and s.sitenumber='"+location+"'";
	...
	public void afterMethod(){
		...
		if(resultFlag.equals("pass")){
			Excel.setCellValBasedOnTcname(path, "CustomerSearch","CustomerSearch_AdvancedSearchUsingLocationNoByPhoneNo_FormerCust" , "Status", "Pass");

		}else if (resultFlag.equals("fail")){

			Excel.setCellValBasedOnTcname(path, "CustomerSearch","CustomerSearch_AdvancedSearchUsingLocationNoByPhoneNo_FormerCust" , "Status", "Fail");
		}else{
			Excel.setCellValBasedOnTcname(path, "CustomerSearch","CustomerSearch_AdvancedSearchUsingLocationNoByPhoneNo_FormerCust" , "Status", "Skip");
		}
