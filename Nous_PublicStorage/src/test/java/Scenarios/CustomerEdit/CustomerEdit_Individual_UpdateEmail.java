...
public class CustomerEdit_Individual_UpdateEmail extends Browser_Factory {
	...
	public Object[][] getCustomerEdit() {
		return Excel.getCellValue_inlist(path, "CustomerEdit", "CustomerEdit", "CustomerEdit_Individual_UpdateEmail");
	}

	@Test(dataProvider = "getCustomerEdit")
	public void CustomerEditIndividualEmail(Hashtable<String, String> tabledata) throws InterruptedException {
		...
		try {
			...
			logger = extent.startTest("CustomerEdit_Individual_UpdateEmail");
...
if(driver.findElements(By.xpath("//div[@id='documents-grid']//table/tbody//tr/td[text()='"+strTodaysDate+"']/following-sibling::td[contains(text(),'Contact Information Changed')]")).size()!=0)
{
	...
	public void afterMethod() {
		...
		if (resultFlag.equals("pass")) {
			Excel.setCellValBasedOnTcname(path, "CustomerEdit", "CustomerEdit_Individual_UpdateEmail", "Status",
					"Pass");
