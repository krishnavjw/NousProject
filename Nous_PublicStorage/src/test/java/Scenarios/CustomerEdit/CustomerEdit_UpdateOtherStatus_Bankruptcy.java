...
public class CustomerEdit_UpdateOtherStatus_Bankruptcy extends Browser_Factory {
	...
	public Object[][] getCustomerEditData() {
		return Excel.getCellValue_inlist(path, "CustomerEdit", "CustomerEdit",
				"CustomerEdit_UpdateOtherStatus_Bankruptcy");
	}

	@Test(dataProvider = "getCustomerEditData")
	public void CustomerEdit_UpdateOtherStatus_Bankruptcy(Hashtable<String, String> tabledata)
			throws InterruptedException {
		try {
			logger = extent.startTest("CustomerEdit_UpdateOtherStatus_Bankruptcy",
					"CustomerEdit_UpdateOtherStatus_Bankruptcy starts ");
		...
		/*	// Select location
			...
			String sqlQuery = "Select  top 1 A.AccountNumber from Account A with(nolock) "
					...
					+ "left join Type T1 with(nolock) on T1.typeid=cmi.militarytypeid " + "Where 1=1 "
					+ "And SOI.VacateDate is Null " + "And CU.CustomerTypeID=90 " + "And CU.IsBankrupt = 0 "
			...
			if (AccDetails.getCustomerInfoTabTxt().contains("Customer Info")) {
				...
				logger.log(LogStatus.PASS,
						"Customer Info tab displayed successfully: " + AccDetails.getCustomerInfoTabTxt());
			...
			if (AccDetails.getSpaceDetailsTabTxt().contains("Space Details")) {
				...
				logger.log(LogStatus.PASS,
						"Space Details tab displayed successfully: " + AccDetails.getSpaceDetailsTabTxt());
			...
			if (AccDetails.getAccountActivitiesTabTxt().contains("Account Activities")) {
				...
				logger.log(LogStatus.PASS,
						"Account Activitiestab displayed successfully: " + AccDetails.getAccountActivitiesTabTxt());
			...
			driver.findElement(By.xpath("//span[text()='Bankruptcy Status']/preceding-sibling::span")).click();
			
			logger.log(LogStatus.PASS, "Clicked on Bankruptcy   radio button successfully ");
			Thread.sleep(2000);
			...
			/*if (otherStatuses.OtherStatusesTitleDisplayed() && otherStatuses.Bankruptcy_txtDisplayed()
					&& otherStatuses.Deceased_txtDisplayed() && otherStatuses.DoNotRent_txtDisplayed()) {
				...
				String image = logger.addScreenCapture(scpath);
				logger.log(LogStatus.PASS,
						"Other Statuses Title ,Bankruptcy ,Deceased,Do Not Rent displayed successfully ");
			...
			otherStatuses.click_declaredCheckbox();
			logger.log(LogStatus.PASS, "Clicked on Declared checkbox successfully ");

			Thread.sleep(4000);

			// Verify Type Dropdown displayed
			if (otherStatuses.typeDropdownDisplayed()) {
				Thread.sleep(2000);
				String scpath = Generic_Class.takeScreenShotPath();
				Reporter.log(scpath, true);
				String image = logger.addScreenCapture(scpath);
				logger.log(LogStatus.PASS, "Type dropdown menu displayed successfully ");
				logger.log(LogStatus.PASS, " Type dropdown menu displayed successfully ", image);
			}
				...
				Reporter.log(scpath, true);
				String image = logger.addScreenCapture(scpath);
				logger.log(LogStatus.FAIL, "Type dropdown menu not  displayed successfully ");
				logger.log(LogStatus.FAIL, "Type dropdown menu not   displayed successfully ", image);
			}

			otherStatuses.select_BankRuptcyType();
			logger.log(LogStatus.PASS, "Bankruptcy Type selected ");
			...
			if (otherStatuses.get_InfoMessageTxt()
					.contains("Access to space is allowed regardless of payment status")) {

				logger.log(LogStatus.PASS,
						"Access to space is allowed regardless of payment status message: message is displayed  successfully ");
				...
				logger.log(LogStatus.FAIL,
						"Access to space is allowed regardless of payment status message: message not  displayed  successfully ");
				logger.log(LogStatus.FAIL,
						"Access to space is allowed regardless of payment status message: message not  displayed  successfully ",
						image);
			}
			...
			driver.findElement(By.xpath("//textarea[@id='BankruptcyNote']")).sendKeys("Bankruptcy Note");
			Thread.sleep(2000);
			...
			String sqlQuery1 = "select bd.isactive, c.isbankrupt from vw_unitdetails u "
					+ "join BankruptcyDeclaration BD on bd.customerid = u.customerid "
					+ "join customer c on c.customerid = u.customerid " + "where  u.accountnumber = '" + AccountNumber
					+ "'";

			ArrayList<String> BankruptcyStatus = DataBase_JDBC.executeSQLQuery_List(sqlQuery1);

			String BankruptcyActiveStatus = BankruptcyStatus.get(0);

			String BankruptcyStatus1 = BankruptcyStatus.get(1);

			if (BankruptcyActiveStatus.equals("1") && BankruptcyStatus1.equals("1")) {
				logger.log(LogStatus.PASS,
						" Both the columns isactive and isbankrupt fetched from DB and hold the value 1");
			}

			else {

				logger.log(LogStatus.FAIL, "Both the columns isactive and isbankrupt not holding value as 1 ");
			}
	...
	public void afterMethod() {
		...
		if (resultFlag.equals("pass")) {
			Excel.setCellValBasedOnTcname(path, "CustomerEdit", "CustomerEdit_UpdateOtherStatus_Bankruptcy", "Status",
					"Pass");
