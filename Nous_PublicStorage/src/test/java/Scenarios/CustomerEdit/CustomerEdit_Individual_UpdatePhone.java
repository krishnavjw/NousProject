...
public class CustomerEdit_Individual_UpdatePhone extends Browser_Factory {
	...
	public Object[][] getCustomerEdit() {
		return Excel.getCellValue_inlist(path, "CustomerEdit", "CustomerEdit", "CustomerEdit_Individual_UpdatePhone");
	}

	@Test(dataProvider = "getCustomerEdit")
	public void CustomerEditIndividualPhone(Hashtable<String, String> tabledata) throws InterruptedException {
		...
		try {
			...
			logger = extent.startTest("CustomerEdit_Individual_UpdatePhone", "Customer Edit Individual - Phone");
...
driver.get(Generic_Class.getPropertyValue("CustomerScreenPath_QA"));
			...
			String phoneBeforeUpdate = driver.findElement(By.xpath("(//div[@class='phones-container clearfix-container']/div[contains(text(),'Phone:')]/following-sibling::div/div/div)[1]/span")).getText();
			...
			// Clicking on Verify button and Confirm with customer
						...
						String phoneAfterUpdate = driver.findElement(By.xpath("(//div[@class='phones-container clearfix-container']/div[contains(text(),'Phone:')]/following-sibling::div/div/div)[1]/span")).getText();
						...
						if(phoneBeforeUpdate.equals(phoneAfterUpdate)){
							...
							logger.log(LogStatus.FAIL, "Phone not Updated  : Phone Before Update is :" + phoneBeforeUpdate  +" Phone After update is : " + phoneAfterUpdate );
							String scpath = Generic_Class.takeScreenShotPath();
							String image = logger.addScreenCapture(scpath);
							logger.log(LogStatus.INFO, "Image", image);
						} else {
							logger.log(LogStatus.PASS, "Phone  Updated successfully in Customer Dashboard : Phone Before Update is :" + phoneBeforeUpdate  +" Phone After update is : " + phoneAfterUpdate );
							String scpath = Generic_Class.takeScreenShotPath();
							String image = logger.addScreenCapture(scpath);
							logger.log(LogStatus.INFO, "Image", image);
							if (resultFlag.equals("pass"))
			    ...
			    Thread.sleep(4000); 
			     ...
			     if(resultFlag.equals("pass"))
					...
					String sqlQuery = "select ph.phonenumber from account a "+
                            ...
                            "join phone ph on ph.phoneid = ap.phoneid "+
			...
			// Verify Customer Info and Account Activities tabs on customer
			// Dashboard

			String getText = driver.findElement(By.xpath("//div[@id='activities-grid']//table/tbody")).getText();
			if (getText.contains("Official contact info changed by ")) {

				logger.log(LogStatus.PASS, "Customer Dashboard is reflected updated data.");
				String scpath = Generic_Class.takeScreenShotPath();
				String image = logger.addScreenCapture(scpath);
				logger.log(LogStatus.INFO, "Image", image);
			} else {
				logger.log(LogStatus.FAIL, "Customer Dashboard is not  reflected updated data.");
				String scpath = Generic_Class.takeScreenShotPath();
				String image = logger.addScreenCapture(scpath);
				logger.log(LogStatus.INFO, "Image", image);
				if (resultFlag.equals("pass"))
			...
			// Verify in the customer Dashboard with the database queries
			String query = "select count(*) from phone where phoneNumber='" + tabledata.get("PhoneNumber") + "'";
			String PhoneRecord = DataBase_JDBC.executeSQLQuery(query);
			Thread.sleep(2000);

			int totalRecord = Integer.parseInt(PhoneRecord);
			if (totalRecord > 0) {
				logger.log(LogStatus.PASS, "Phone Updated successfully in DB : " + tabledata.get("PhoneNumber"));
			}
			*/
	...
	public void afterMethod() {
		...
		if (resultFlag.equals("pass")) {
			Excel.setCellValBasedOnTcname(path, "CustomerEdit", "CustomerEdit_Individual_UpdatePhone", "Status",
					"Pass");
