...
import Pages.Walkin_Reservation_Lease.Fudiciary_Information;
...
public class MakePayment_Business_NewCustomer_MultipleSpaces_paythroughCash extends Browser_Factory{
	...
	{
		return Excel.getCellValue_inlist(path, "Payments","Payments","MakePayment_Business_NewCustomer_MultipleSpaces_paythroughCash");
	}

	@Test(dataProvider="getCustSearchData")
	public void MakePayment_Business_NewCustomer_MultipleSpaces_paythroughCash(Hashtable<String, String> tabledata) throws InterruptedException 
	{
		try{
			logger=extent.startTest("MakePayment_Business_NewCustomer_MultipleSpaces_paythroughCheck", "MakePayment_Business_NewCustomer_MultipleSpaces_paythroughCheck ");
			...
			Thread.sleep(8000);
			//select yes radio btn
			contactinfo.clk_legallyonBehlf_Cmpny();
			Thread.sleep(5000);
		   ...
		   //switching back to wc2 tab
			 ...
			 auth.clk_savebutton();//clk_saveAndProceedbutton();
			 Thread.sleep(10000);
			 ...
			 Leasing_LeaseQuestionairePage leaseQues= new Leasing_LeaseQuestionairePage(driver);
				...
				logger.log(LogStatus.INFO, "clicked yes Insurance");
				
				 Thread.sleep(5000);
		         leaseQues.clickCoverageList();
		         logger.log(LogStatus.INFO, "clicked coverage list");
		         ...
		         Thread.sleep(6000);
		        // String insurance="$11.00 ($3,000 coverage)";
		         for(WebElement coverage: allcoverage)
		         {
		                System.out.println("the inside is---");
		                if((tabledata.get("InsuranceCoverage")).contains(coverage.getText()))
		                {
		                       System.out.println("the inside is dsfd---");
		                       coverage.click();
		                       break;
		                }
		         }
		         logger.log(LogStatus.INFO, "Selected desired coverage");
				...
				Fudiciary_Information fud=new Fudiciary_Information(driver);
				fud.click_saveProceed_btn();
				Thread.sleep(10000);
				logger.log(LogStatus.PASS,"Clicked on save and proceed button ");
				...
				driver.findElement(By.xpath("//div[@class='k-list-container k-popup k-group k-reset k-state-border-down']/ul/li[text()='Cash']")).click();
				logger.log(LogStatus.PASS,"selected Cash option option");

				
				  //get total Amount due
			    String amount= payment.get_Amount();
			    
				 Thread.sleep(3000);
	               payment.enter_CashAmount(amount);
	               Thread.sleep(3000);
	...
	public void afterMethod(){
		...
		if(resultFlag.equals("pass")){
			Excel.setCellValBasedOnTcname(path, "Payments","MakePayment_Business_NewCustomer_MultipleSpaces_paythroughCash" , "Status", "Pass");

		}else if (resultFlag.equals("fail")){

			Excel.setCellValBasedOnTcname(path, "Payments","MakePayment_Business_NewCustomer_MultipleSpaces_paythroughCash" , "Status", "Fail");
		}else{
			Excel.setCellValBasedOnTcname(path, "Payments","MakePayment_Business_NewCustomer_MultipleSpaces_paythroughCash" , "Status", "Skip");
		}
