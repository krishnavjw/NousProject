...
public class Payments_NewIndCustSingleSpace_Cash_AP extends Browser_Factory {
	...
	public void Payments_NewIndCustSingleSpace_Cash_AP(Hashtable<String, String> tabledata) throws Exception {
		...
		try {
			...
			logger = extent.startTest("Payments_NewIndCustSingleSpace_Cash_AP","New individual customer make payment using cash advance pay");
	...
	public void afterMethod() {
		...
		if (resultFlag.equals("pass")) {
			Excel.setCellValBasedOnTcname(path, "Payments", "Payments_NewIndCustSingleSpace_Cash_AP", "Status","Pass");
		} else if (resultFlag.equals("fail")) {
			String scpath = Generic_Class.takeScreenShotPath();
			String image = logger.addScreenCapture(scpath);
			logger.log(LogStatus.FAIL, " page is not displayed", image);
			Excel.setCellValBasedOnTcname(path, "Payments", "Payments_NewIndCustSingleSpace_Cash_AP", "Status","Fail");
		} else {
			Excel.setCellValBasedOnTcname(path, "Payments", "Payments_NewIndCustSingleSpace_Cash_AP", "Status","Skip");
		}
