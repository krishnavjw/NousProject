...
public class MakePrepayment_PaidThroughDate_RentInsurance extends Browser_Factory {
	...
	public Object[][] getPaymentsData() {
		return Excel.getCellValue_inlist(path, "Payments", "Payments", "MakePrepayment_PaidThroughDate_RentInsurance");
	}

	@Test(dataProvider = "getPaymentsData")
	public void MakePrepayment_PaidThroughDate_RentInsurance(Hashtable<String, String> tabledata) throws InterruptedException {
		...
		try {
			logger = extent.startTest("MakePrepayment_PaidThroughDate_RentInsurance","Prepayment_PaidThruDate_RentInsurance starts ");
			...
			if (cust_accdetails.getCustDashboardTitle().contains("Confirm Customer's Identity and Contact Information")) {
				...
				logger.log(LogStatus.PASS,"Customer Account Dashboard displayed successfully for an Individual Custome :"
			...
			// Select PayThrough
			payments.selectPaymentThrough(1);
			Thread.sleep(5000);
			logger.log(LogStatus.PASS, "pay Through selected successfully ");

			double monthlyRentAmount = Double.parseDouble(payments.getMonthlyRentAmount().substring(1));
			logger.log(LogStatus.INFO, "Fetching the Monthlt rent Amount :" + monthlyRentAmount);
			Thread.sleep(2000);

			double insuranceAmount = Double.parseDouble(payments.getInsuranceAmount().substring(1));
			logger.log(LogStatus.INFO, "Fetching the Insurance Amount :" + insuranceAmount);
			Thread.sleep(2000);

			String total = driver
					.findElement(By
							.xpath("//div[@id='payment-table']//section[@class='row orderTotal']/div[text()='Total']/following-sibling::div[@class='second col']/span"))
					.getText();

			double totalAmount = Double.parseDouble(total.substring(1));

			if (totalAmount == monthlyRentAmount + insuranceAmount) {
				...
				logger.log(LogStatus.PASS,
						"displayed total is sum of Monthly Rent and Insurance amount: " + "Monthly rent is :"
								+ monthlyRentAmount + "Insurance is : " + insuranceAmount + "Total Amount is :"
								+ totalAmount);
				logger.log(LogStatus.PASS, "displayed total is sum of Monthly Rent and Insurance amount:", image);
				...
				String image = logger.addScreenCapture(scpath);
				logger.log(LogStatus.PASS,
						" Not displayed total is sum of Monthly Rent and Insurance amount: " + "Monthly rent is :"
								+ monthlyRentAmount + "Insurance is : " + insuranceAmount + "Total Amount is :"
								+ totalAmount);
				logger.log(LogStatus.PASS, " Not displayed total is sum of Monthly Rent and Insurance amount:", image);
			...
			logger.log(LogStatus.INFO, "Select the cash option from Payment dropdown successfully");
	...
	public void afterMethod() {
		...
		if (resultFlag.equals("pass")) {
			Excel.setCellValBasedOnTcname(path, "Payments", "MakePrepayment_PaidThroughDate_RentInsurance", "Status", "Pass");

		} else if (resultFlag.equals("fail")) {

			Excel.setCellValBasedOnTcname(path, "Payments", "MakePrepayment_PaidThroughDate_RentInsurance", "Status", "Fail");
		} else {
			Excel.setCellValBasedOnTcname(path, "Payments", "MakePrepayment_PaidThroughDate_RentInsurance", "Status", "Skip");
		}
