...
public class Payments_NewIndCustSingleSpace_Check_AP extends Browser_Factory{
	...
	public Object[][] getLoginData() {
		return Excel.getCellValue_inlist(path, "Payments", "Payments","Payments_NewIndCustSingleSpace_Check_AP");
	}

	@Test(dataProvider = "getLoginData")
	public void Payments_NewIndCustSingleSpace_Check_AP(Hashtable<String, String> tabledata) throws Exception {

		if (!(tabledata.get("Runmode").equals("Y") && tabledata.get("Payments").equals("Y"))) {
			resultFlag = "skip";
			logger.log(LogStatus.SKIP, "Payments_NewIndCustSingleSpace_Check_AP is Skipped");
			throw new SkipException("Skipping the test Payments_NewIndCustSingleSpace_Check_AP");
		}
		...
		try {
			...
			logger = extent.startTest("Payments_NewIndCustSingleSpace_Check_AP","New individual customer make payment using Check advance pay");
			...
			Thread.sleep(1000);
			String totalDueNowAmount = cust_accdetails.getTotalDue();
	...
	public void afterMethod() {
		...
		if (resultFlag.equals("pass")) {
			Excel.setCellValBasedOnTcname(path, "Payments", "Payments_NewIndCustSingleSpace_Check_AP", "Status","Pass");
		} else if (resultFlag.equals("fail")) {
			String scpath = Generic_Class.takeScreenShotPath();
			String image = logger.addScreenCapture(scpath);
			logger.log(LogStatus.FAIL, " page is not displayed", image);
			Excel.setCellValBasedOnTcname(path, "Payments", "Payments_NewIndCustSingleSpace_Check_AP", "Status","Fail");
		} else {
			Excel.setCellValBasedOnTcname(path, "Payments", "Payments_NewIndCustSingleSpace_Check_AP", "Status","Skip");
		}
