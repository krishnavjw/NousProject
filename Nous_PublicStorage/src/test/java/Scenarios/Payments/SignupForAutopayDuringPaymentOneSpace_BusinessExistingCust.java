...
public class SignupForAutopayDuringPaymentOneSpace_BusinessExistingCust extends Browser_Factory {
	...
	{
		return Excel.getCellValue_inlist(path, "Payments","Payments", "SignupForAutopayDuringPaymentOneSpace_BusinessExistingCust");
	}
	
	
	@Test(dataProvider="getData")
	public void SignupForAutopayDuringPaymentOneSpace_BusinessExistingCust(Hashtable<String, String> tabledata) throws InterruptedException
	{
		logger=extent.startTest("SignupForAutopayDuringPaymentOneSpace_BusinessExistingCust","Signup For Autopay During Payment One Space");

		if(!(tabledata.get("Runmode").equals("Y") && tabledata.get("Payments").equals("Y")))
		{
			resultFlag="skip";
			logger.log(LogStatus.SKIP, "SignupForAutopayDuringPaymentOneSpace_BusinessExistingCust is Skipped");
		...
		try
		{
			...
			if(emailIdUI.contains(DB_Data.get(0))&addressUI.contains(DB_Data.get(1))&addressUI.contains(DB_Data.get(2))&addressUI.contains(DB_Data.get(3))&addressUI.contains(DB_Data.get(4))&identificationUI.contains(DB_Data.get(5))){
				...
				logger.log(LogStatus.PASS, "Actual Values: Email-" + DB_Data.get(0) + " Actual Values: Address-" + DB_Data.get(1) + DB_Data.get(2) +DB_Data.get(3) + DB_Data.get(4) + " Actual Values: identification-" + DB_Data.get(5));
			...
			else
			{
				...
				logger.log(LogStatus.FAIL, "Actual Values: Email-" + DB_Data.get(0) + " Actual Values: Address-" + DB_Data.get(1) + DB_Data.get(2) +DB_Data.get(3) + DB_Data.get(4) + " Actual Values: identification-" + DB_Data.get(5));
			}
			Thread.sleep(8000);
	...
	public void afterMethod(){
		...
		if(resultFlag.equals("pass")){
			Excel.setCellValBasedOnTcname(path,"Payments","SignupForAutopayDuringPaymentOneSpace_BusinessExistingCust" , "Status", "Pass");

		}else if (resultFlag.equals("fail")){    
			Excel.setCellValBasedOnTcname(path,"Payments","SignupForAutopayDuringPaymentOneSpace_BusinessExistingCust" , "Status", "Fail");
		}else{
			Excel.setCellValBasedOnTcname(path, "Payments","SignupForAutopayDuringPaymentOneSpace_BusinessExistingCust" , "Status", "Skip");
		}
