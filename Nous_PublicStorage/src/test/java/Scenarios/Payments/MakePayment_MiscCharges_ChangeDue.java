...
public class MakePayment_MiscCharges_ChangeDue extends Browser_Factory{
	...
	public void MakePayment_MiscCharges_ChangeDue(Hashtable<String, String> tabledata) throws InterruptedException 
	{
		logger=extent.startTest("MakePayment_MiscCharges_ChangeDue","Make payment for a customer who incurred misc charges with Change Due");
		...
		try{
			...
			payments.enter_CashAmount(Double.toString(dTotalDue));
			Thread.sleep(2000);
			...
			if(getTotalDue.contains("$0.00 ")){
				...
				scpath=Generic_Class.takeScreenShotPath();
				image=logger.addScreenCapture(scpath);
				logger.log(LogStatus.FAIL, "Customer total due is zero");
				logger.log(LogStatus.FAIL, "Image",image);
				if(resultFlag.equals("pass"))
					resultFlag="fail";
			}
