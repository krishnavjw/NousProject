...
public class EnrollIntoAutopayThruLeasing_Individual_WithInsurance_WithoutTax_Check extends Browser_Factory{
	...
	{
		return Excel.getCellValue_inlist(path,"AutoPay","AutoPay",  "EnrollIntoAutopayThruLeasing_Individual_WithInsurance_WithoutTax_Check");
	}



	@Test(dataProvider="getLoginData")
	public void EnrollIntoAutopayThruLeasing_Individual_WithInsurance_WithoutTax_Check(Hashtable<String, String> tabledata) throws InterruptedException 
	{
		...
		try
		{
			...
			logger=extent.startTest("EnrollIntoAutopayThruLeasing_Individual_WithInsurance_WithoutTax_Check","Enroll Into Autopay Thru Leasing _Individual_WithInsurance_WithoutTax_Check");
		   ...
		   if(driver.findElements(By.xpath("//div[@id='cfsConnectionDialog']//div[@class='after-connected padding-top']//p/input[@class='psbutton-low-priority']")).size()!=0){
			...
			logger.log(LogStatus.PASS, "Account Created Successfully :" +accNUm);
			scpath=Generic_Class.takeScreenShotPath();
			image=logger.addScreenCapture(scpath);
			...
			if(driver.findElements(By.xpath("//div[@id='documents-grid']//table/tbody//tr/td[text()='"+strTodaysDate+"']/following-sibling::td[text()='"+avlSpace+"']/preceding-sibling::td[text()='Propertywalkin AutoPay generated contract']")).size()!=0)
			{
				scpath=Generic_Class.takeScreenShotPath();
				image=logger.addScreenCapture(scpath);
	...
	public void afterMethod(){
		...
		if(resultFlag.equals("pass")){
			Excel.setCellValBasedOnTcname(path, "AutoPay","EnrollIntoAutopayThruLeasing_Individual_WithInsurance_WithoutTax_Check" , "Status", "Pass");

		}else if (resultFlag.equals("fail")){

			Excel.setCellValBasedOnTcname(path, "AutoPay","EnrollIntoAutopayThruLeasing_Individual_WithInsurance_WithoutTax_Check" , "Status", "Fail");
		}else{
			Excel.setCellValBasedOnTcname(path, "AutoPay","EnrollIntoAutopayThruLeasing_Individual_WithInsurance_WithoutTax_Check" , "Status", "Skip");
		}
