...
public class EnrollIntoAutopay_Individual_CreditCard extends Browser_Factory {
	...
	public Object[][] getLoginData() 
	{
		...
		return Excel.getCellValue_inlist(path, "AutoPay","AutoPay", "EnrollIntoAutopay_Individual_CreditCard");
	}

	@Test(dataProvider="getLoginData")
	public void EnrollIntoAutopay_Individual_CreditCard(Hashtable<String, String> tabledata) throws InterruptedException
	{

		logger=extent.startTest("EnrollIntoAutopay_Individual_CreditCard","Enroll into autopay with credit card for Individual customer");

		if(!(tabledata.get("Runmode").equals("Y") && tabledata.get("AutoPay").equals("Y")))
		{
			resultFlag="skip";
			logger.log(LogStatus.SKIP, "EnrollIntoAutopay_Individual_CreditCard is Skipped");
			throw new SkipException("Skipping the test EnrollIntoAutopay_Ind_CreditCard");
		}
		...
		try
		{
			...
			/*String query="Select Top 1 A.AccountNumber "+
					...
					"and AP.isActive=0 "+
					"and AP.AccountCreditCardID is null "+
					"and AP.AccountCheckID is null "+
					"and t1.name<>'cancel' "+
			...
			//logger.log(LogStatus.INFO, "Fetching customer sapce number and space number is:"+spaceNum);
			Thread.sleep(6000);
	...
	public void afterMethod(){
		...
		if(resultFlag.equals("pass")){
			Excel.setCellValBasedOnTcname(path,"AutoPay","EnrollIntoAutopay_Individual_CreditCard" , "Status", "Pass");

		}else if (resultFlag.equals("fail")){    
			Excel.setCellValBasedOnTcname(path,"AutoPay","EnrollIntoAutopay_Individual_CreditCard" , "Status", "Fail");
		}else{
			Excel.setCellValBasedOnTcname(path, "AutoPay","EnrollIntoAutopay_Individual_CreditCard" , "Status", "Skip");
		}
