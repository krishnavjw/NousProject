...
public class AutoPayEnrollmentViaMakePayment_Individual_CreditCard extends Browser_Factory{
	...
	{
		return Excel.getCellValue_inlist(path, "AutoPay","AutoPay", "AutoPayEnrollmentViaMakePayment_Individual_CreditCard");
	}


	@Test(dataProvider="getData")
	public void AutoPayEnrollmentViaMakePayment_Individual_CreditCard(Hashtable<String, String> tabledata) throws InterruptedException
	{
		logger=extent.startTest("AutoPayEnrollmentViaMakePayment_Individual_CreditCard","AutoPay Enrollment via Make Payment - Individual Customer - Check Payment");
		...
		try
		{
			...
			Reporter.log(scpath,true);
			image=logger.addScreenCapture(scpath);
			logger.log(LogStatus.PASS, "Credit Card and amount details entered successfully");
			logger.log(LogStatus.INFO, "Card and amount details entered",image);
			Thread.sleep(4000);
			...
			scpath=Generic_Class.takeScreenShotPath();
			image=logger.addScreenCapture(scpath);
			logger.log(LogStatus.PASS, "Navigate to customer dashboard");
			logger.log(LogStatus.INFO, "Image",image);
			...
			if(driver.findElement(By.xpath(xpath)).isDisplayed())
			{
				...
				logger.log(LogStatus.INFO, "Autopay status enabled",image);
			}else{
				scpath=Generic_Class.takeScreenShotPath();
				image=logger.addScreenCapture(scpath);
				logger.log(LogStatus.FAIL, "Auto pay Status not enabled");
				logger.log(LogStatus.INFO, "Autopay status not enabled",image);
				if(resultFlag.equals("pass"))
					resultFlag="fail";
			}
	...
	public void afterMethod(){
		...
		if(resultFlag.equals("pass")){
			Excel.setCellValBasedOnTcname(path,"AutoPay","AutoPayEnrollmentViaMakePayment_Individual_CreditCard" , "Status", "Pass");

		}else if (resultFlag.equals("fail")){    
			Excel.setCellValBasedOnTcname(path,"AutoPay","AutoPayEnrollmentViaMakePayment_Individual_CreditCard" , "Status", "Fail");
		}else{
			Excel.setCellValBasedOnTcname(path, "AutoPay","AutoPayEnrollmentViaMakePayment_Individual_CreditCard" , "Status", "Skip");
		}
