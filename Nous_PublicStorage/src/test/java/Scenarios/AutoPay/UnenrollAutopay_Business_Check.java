...
public class UnenrollAutopay_Business_Check extends Browser_Factory {
	...
	public Object[][] getLoginData() {
		...
		return Excel.getCellValue_inlist(path, "AutoPay", "AutoPay", "UnenrollAutopay_Business_Check");
	}

	@Test(dataProvider = "getLoginData")
	public void UnenrollAutopay_Business_Check(Hashtable<String, String> tabledata) throws InterruptedException {
		logger = extent.startTest("UnenrollAutopay_Business_Check","UnEnroll/stop/cancel Autopay for an Business customer ");

		if (!(tabledata.get("Runmode").equals("Y") && tabledata.get("AutoPay").equals("Y"))) {
			resultFlag = "skip";
			logger.log(LogStatus.SKIP, "UnenrollAutopay_Business_Check is Skipped");
		...
		try {
			...
			if(checkStatus.contains("AutoPay not enabled for this space")){
				scpath=Generic_Class.takeScreenShotPath();
				image=logger.addScreenCapture(scpath);
				logger.log(LogStatus.PASS, "AutoPay Status is disabled for the space after unenrollment");
				logger.log(LogStatus.INFO, "AutoPay status is disabled after unenrollment",image);
			}else{
				scpath=Generic_Class.takeScreenShotPath();
				image=logger.addScreenCapture(scpath);
				logger.log(LogStatus.FAIL, "AutoPay Status is not disabled");
				logger.log(LogStatus.INFO, "AutoPay Status is not disabled",image);
				if(resultFlag.equals("pass"))
					resultFlag="fail";
			}
			
			Thread.sleep(20000);
			if(driver.findElements(By.xpath("//a[@class='p"
					+ "sbutton-priority margin-left ok-button']")).size()!=0){
				driver.findElement(By.xpath("//a[@class='psbutton-priority margin-left ok-button']")).click();
				scpath=Generic_Class.takeScreenShotPath();
				image=logger.addScreenCapture(scpath);
				logger.log(LogStatus.PASS, "Alert Ok popup displyed");
	...
	public void afterMethod() {
		...
		if (resultFlag.equals("pass")) {
			Excel.setCellValBasedOnTcname(path, "AutoPay", "UnenrollAutopay_Business_Check", "Status", "Pass");

		} else if (resultFlag.equals("fail")) {
			// String scpath=Generic_Class.takeScreenShotPath();
			// String image=logger.addScreenCapture(scpath);
			// logger.log(LogStatus.FAIL, "Create Reservation page is not
			// displayed",image);
			Excel.setCellValBasedOnTcname(path, "AutoPay", "UnenrollAutopay_Business_Check", "Status", "Fail");
		} else {
			Excel.setCellValBasedOnTcname(path, "AutoPay", "UnenrollAutopay_Business_Check", "Status", "Skip");
		}
