...
public class Enroll_autopay_leasing_business_CreditCard_without_insurance_without_tax extends Browser_Factory{
	...
	public Object[][] getLoginData() 
	{
		...
		return Excel.getCellValue_inlist(path, "AutoPay","AutoPay", "Enroll_autopay_leasing_business_CreditCard_without_insurance_without_tax");
	}


	@Test(dataProvider="getLoginData")
	public void Enroll_autopay_leasing_business_CreditCard_without_ins_without_tax(Hashtable<String, String> tabledata) throws InterruptedException
	{

		if(!(tabledata.get("Runmode").equals("Y") && tabledata.get("AutoPay").equals("Y")))
		{
			resultFlag="skip";
			logger.log(LogStatus.SKIP, "Enroll_autopay_leasing_business_CreditCard_without_insurance_without_tax is Skipped");
		...
		try
		{
			...
			logger=extent.startTest("Enroll_autopay_leasing_business_CreditCard_without_insurance_without_tax","Enroll Into Autopay Thru Leasing _Individual_WithInsurance_WithoutTax_Check");
		   ...
		   if(driver.findElements(By.xpath("//div[@id='cfsConnectionDialog']//div[@class='after-connected padding-top']//p/input[@class='psbutton-low-priority']")).size()!=0){
			...
			logger.log(LogStatus.PASS, "Account Number Created Successfully : " +accNUm);
			scpath=Generic_Class.takeScreenShotPath();
			image=logger.addScreenCapture(scpath);
			...
			if(driver.findElements(By.xpath("//div[@id='activities-grid']//table//tbody//tr//td[text()='"+strTodaysDate+"']/following-sibling::td[text()='Autopay']/..//td/div[contains(text(),'Autopay turned on')]")).size()!=0)
			{
				scpath=Generic_Class.takeScreenShotPath();
				image=logger.addScreenCapture(scpath);
				logger.log(LogStatus.PASS, "AutoPay Status is enabled for the space in Account Activities");
				logger.log(LogStatus.INFO, "Image",image);
				...
				scpath=Generic_Class.takeScreenShotPath();
				image=logger.addScreenCapture(scpath);
				logger.log(LogStatus.FAIL, "AutoPay Status is not enabled in Account Activities");
				logger.log(LogStatus.INFO, "Image",image);
	...
	public void afterMethod(){
		...
		if(resultFlag.equals("pass")){
			Excel.setCellValBasedOnTcname(path, "AutoPay","Enroll_autopay_leasing_business_CreditCard_without_insurance_without_tax" , "Status", "Pass");

		}else if (resultFlag.equals("fail")){

			Excel.setCellValBasedOnTcname(path, "AutoPay","Enroll_autopay_leasing_business_CreditCard_without_insurance_without_tax" , "Status", "Fail");
		}else{
			Excel.setCellValBasedOnTcname(path, "AutoPay","Enroll_autopay_leasing_business_CreditCard_without_insurance_without_tax" , "Status", "Skip");
		}
