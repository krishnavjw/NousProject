...
public class AutoPayEnrollmentViaMakePayment_Individual_Check extends Browser_Factory{
	...
	{
		return Excel.getCellValue_inlist(path, "AutoPay","AutoPay", "AutoPayEnrollmentViaMakePayment_Individual_Check");
	}


	@Test(dataProvider="getData")
	public void AutoPayEnrollmentViaMakePayment_Individual_Check(Hashtable<String, String> tabledata) throws InterruptedException
	{
		logger=extent.startTest("AutoPayEnrollmentViaMakePayment_Individual_Check","AutoPay Enrollment via Make Payment - Individual Customer - Check Payment");

		if(!(tabledata.get("Runmode").equals("Y") && tabledata.get("AutoPay").equals("Y")))
		{
			resultFlag="skip";
			logger.log(LogStatus.SKIP, "AutoPayEnrollmentViaMakePayment_Individual_Check is Skipped");
		...
		try
		{
			...
			logger.log(LogStatus.INFO, "Check Details entered",image);
			Thread.sleep(4000);
			...
			if(spaceTab.verifyAutopay_Status())
			{
				scpath=Generic_Class.takeScreenShotPath();
				image=logger.addScreenCapture(scpath);
				logger.log(LogStatus.PASS, "Autopay Status is ON and enrolled in Space Details Tab");
				logger.log(LogStatus.INFO, "Autopay Status is ON and enrolled ",image);

			}else{
				scpath=Generic_Class.takeScreenShotPath();
				image=logger.addScreenCapture(scpath);
				logger.log(LogStatus.FAIL, "Autopay Status is NOT enrolled in Space Details Tab");
				logger.log(LogStatus.INFO, "Autopay Status is NOT enrolled and enrolled ",image);
			...
			if(driver.findElements(By.xpath("//div[@id='documents-grid']//table/tbody//tr/td[text()='"+strTodaysDate+"']/following-sibling::td[text()='"+spaceNum+"']/preceding-sibling::td[text()='Property AutoPay contract']")).size()!=0)
			{
				scpath=Generic_Class.takeScreenShotPath();
				image=logger.addScreenCapture(scpath);
				logger.log(LogStatus.PASS, "Autopay Addendum validation in Documents Tab");
				logger.log(LogStatus.PASS, "Property AutoPay contract record is successfully displayed in Documents Tab");
				logger.log(LogStatus.INFO, "Image",image);
				...
				scpath=Generic_Class.takeScreenShotPath();
				image=logger.addScreenCapture(scpath);
				logger.log(LogStatus.FAIL, "Property AutoPay contract record is not displayed in Documents Tab");
				logger.log(LogStatus.INFO, "Image",image);
				if(resultFlag.equals("pass"))
					resultFlag="fail";
			}
	...
	public void afterMethod(){
		...
		if(resultFlag.equals("pass")){
			Excel.setCellValBasedOnTcname(path,"AutoPay","AutoPayEnrollmentViaMakePayment_Individual_Check" , "Status", "Pass");

		}else if (resultFlag.equals("fail")){    
			Excel.setCellValBasedOnTcname(path,"AutoPay","AutoPayEnrollmentViaMakePayment_Individual_Check" , "Status", "Fail");
		}else{
			Excel.setCellValBasedOnTcname(path, "AutoPay","AutoPayEnrollmentViaMakePayment_Individual_Check" , "Status", "Skip");
		}
