...
public class EnrollIntoAutopay_Individual_Check extends Browser_Factory {
	...
	public Object[][] getLoginData() 
	{
		...
		return Excel.getCellValue_inlist(path, "AutoPay","AutoPay", "EnrollIntoAutopay_Individual_Check");
	}

	@Test(dataProvider="getLoginData")
	public void EnrollIntoAutopay_Individual_Check(Hashtable<String, String> tabledata) throws InterruptedException
	{

		logger=extent.startTest("EnrollIntoAutopay_Individual_Check","Enroll into autopay with Check for Individual customer");

		if(!(tabledata.get("Runmode").equals("Y") && tabledata.get("AutoPay").equals("Y")))
		{
			resultFlag="skip";
			logger.log(LogStatus.SKIP, "EnrollIntoAutopay_Individual_Check is Skipped");
			throw new SkipException("Skipping the test EnrollIntoAutopay_Ind_Check");
		}
		...
		try
		{
			...
			if(driver.findElements(By.xpath("//div[@id='space-details-grid']//div//table//tbody//tr//td//div//a[contains(text(),'Add AutoPay')]")).size()!=0){
				driver.findElement(By.xpath("//div[@id='space-details-grid']//div//table//tbody//tr//td//div//a[contains(text(),'Add AutoPay')]")).click();
				scpath=Generic_Class.takeScreenShotPath();
				image=logger.addScreenCapture(scpath);
				logger.log(LogStatus.PASS, "Clicking on Add Autopay link to Enroll into Autopay in Space Details tab");
				logger.log(LogStatus.INFO, "Clicking on Add Autopay link to Enroll into Autopay in Space Details tab",image);
			}else{
				logger.log(LogStatus.INFO, "Add Autopay link is not  displyed in Space Details tab");
			}
		...
		Thread.sleep(5000);
			...
			scpath=Generic_Class.takeScreenShotPath();
			image=logger.addScreenCapture(scpath);
			logger.log(LogStatus.PASS, "Enter employee id successfully");
			logger.log(LogStatus.INFO, "Enter employee id successfully",image);
			Thread.sleep(3000);
			empidpage.clk_OkBtn();
	...
	public void afterMethod(){
		...
		if(resultFlag.equals("pass")){
			Excel.setCellValBasedOnTcname(path,"AutoPay","EnrollIntoAutopay_Individual_Check" , "Status", "Pass");

		}else if (resultFlag.equals("fail")){    
			Excel.setCellValBasedOnTcname(path,"AutoPay","EnrollIntoAutopay_Individual_Check" , "Status", "Fail");
		}else{
			Excel.setCellValBasedOnTcname(path, "AutoPay","EnrollIntoAutopay_Individual_Check" , "Status", "Skip");
		}
