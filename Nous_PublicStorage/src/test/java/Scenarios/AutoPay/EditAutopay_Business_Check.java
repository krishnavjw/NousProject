...
public class EditAutopay_Business_Check extends Browser_Factory {
	...
	public Object[][] getLoginData() 
	{
		...
		return Excel.getCellValue_inlist(path, "AutoPay","AutoPay", "EditAutopay_Business_Check");
	}

	@Test(dataProvider="getLoginData")
	public void EditAutopay_Business_Check(Hashtable<String, String> tabledata) throws InterruptedException
	{

		logger=extent.startTest("EditAutopay_Business_Check","Enroll into autopay with Check for business customer");

		if(!(tabledata.get("Runmode").equals("Y") && tabledata.get("AutoPay").equals("Y")))
		{
			resultFlag="skip";
			logger.log(LogStatus.SKIP, "EditAutopay_Business_Check is Skipped");
			throw new SkipException("Skipping the test EditAutopay_Bus_Check");
		}
		...
		try
		{
			...
			if(driver.findElements(By.xpath("//div[@id='activities-grid']//table//tbody//tr//td[text()='"+strTodaysDate+"']/following-sibling::td[text()='Autopay']/..//td/div[contains(text(),'Autopay changed')]")).size()!=0)
			{
				...
				logger.log(LogStatus.FAIL, "AutoPay Status is not changed adn not  enabled - Customer Account Dashboard",image);
				if(resultFlag.equals("pass"))
					resultFlag="fail";
			}
			...
			logger.log(LogStatus.PASS, "Edit autopay for business customer is successful");
	...
	public void afterMethod(){
		...
		if(resultFlag.equals("pass")){
			Excel.setCellValBasedOnTcname(path,"AutoPay","EditAutopay_Business_Check" , "Status", "Pass");

		}else if (resultFlag.equals("fail")){    
			Excel.setCellValBasedOnTcname(path,"AutoPay","EditAutopay_Business_Check" , "Status", "Fail");
		}else{
			Excel.setCellValBasedOnTcname(path, "AutoPay","EditAutopay_Business_Check" , "Status", "Skip");
		}
