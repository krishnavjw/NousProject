...
public class PropertyManagement_UpdateSpaceStatusForSpace_UnrentableOps extends Browser_Factory{
	...
	public void propertyManagement_UpdateSpaceStatusForSpace_UnrentableOps(Hashtable<String, String> tabledata) throws InterruptedException 
	{
		...
		try
		{
			...
			logger=extent.startTest(this.getClass().getSimpleName(),"Property Management - Update Space Status for Space - Unrentable Ops");
			...
			String sqlQuery = "select top 1 ru.rentalunitnumber, ps.siteid from rentalunit ru with(nolock) "+
							  ...
							  " where ru.rentalstatustypeid=57 order by ps.siteid desc";
			...
			if(driver.findElement(By.xpath("//div[@id='employeeNumberEntry']//div[text()='New Status:']/following-sibling::div//span[@class='k-input']")).isDisplayed()){
				scpath=Generic_Class.takeScreenShotPath();
				image=logger.addScreenCapture(scpath);
			...
			found = false;
			String[] expValuesNewStatus = {"Unit Vacant","Unit Occupied By Public Storage"};
			...
			found = false;
			String[] expValuesNewStatus2 = {"Unit Vacant","Unit Occupied By Public Storage"};
