...
public class Verify_Do_Not_Rent_Flag_ImportantInformation extends Browser_Factory {
	...
	{
		return Excel.getCellValue_inlist(path,"CustomerDashBoard","CustomerDashBoard", "Verify_Do_Not_Rent_Flag_ImportantInformation");
	}

	@Test(dataProvider="getLoginData")
	public void Verify_Do_Not_Rent_Flag_ImportantInformation(Hashtable<String, String> tabledata) throws Exception 
	{
		...
		try{
			logger=extent.startTest("Verify_Do_Not_Rent_Flag_ImportantInformation","Customer DashBoard - Verify Do Not Rent Flag in Important Information");
			...
			String updateQuery1="UPDATE CUSTOMER SET DoNotRent = '0' where customerid=(select customerid from account where AccountNumber='"+AccountNumber+"')";
			...
			boolean donotrent = driver.findElements(By.xpath("//span[@class='important-info-title title bold'][text()='DO NOT RENT']")).size()>0;
			...
			if(donotrent == false){
				...
				Reporter.log(scpath, true);
				String image = logger.addScreenCapture(scpath);
				logger.log(LogStatus.PASS, "Do Not Rent Flag is not displayed");
				logger.log(LogStatus.INFO, "Do Not Rent Flag is not displayed", image);
			}else{
				String scpath = Generic_Class.takeScreenShotPath();
				Reporter.log(scpath, true);
				if(resultFlag.equals("pass"))
					resultFlag="fail";
				String image = logger.addScreenCapture(scpath);
				logger.log(LogStatus.FAIL, "Do Not Rent Flag is displayed");
				logger.log(LogStatus.INFO, "Do Not Rent Flag is displayed", image);
			}
			
			
			String updateQuery="UPDATE CUSTOMER SET DoNotRent = '1' where customerid=(select customerid from account where AccountNumber='"+AccountNumber+"')";
			DataBase_JDBC.executeSQLQuery(updateQuery);
			logger.log(LogStatus.INFO, "Do not Rent Flag is updated in Database");
			Thread.sleep(5000);
			...
			//verifying for do not rent flag
			
			((JavascriptExecutor)driver).executeScript("window.scrollTo(2000,0)");
			Thread.sleep(2000);
			
			if(cust_accdetails.verify_DoNotRentFlag()){
				String scpath = Generic_Class.takeScreenShotPath();
				Reporter.log(scpath, true);
				String image = logger.addScreenCapture(scpath);
				logger.log(LogStatus.PASS, "Do Not Rent Flag is displayed successfully");
				logger.log(LogStatus.INFO, "Do Not Rent Flag  is displayed successfully", image);
			}
				...
				if(resultFlag.equals("pass"))
					resultFlag="fail";
				String image = logger.addScreenCapture(scpath);
				logger.log(LogStatus.FAIL, "Do Not Rent Flag is not displayed");
				logger.log(LogStatus.INFO, "Do Not Rent Flag is not displayed", image);
			}
	...
	public void afterMethod(){
		...
		if(resultFlag.equals("pass")){
			Excel.setCellValBasedOnTcname(path, "CustomerDashBoard","Verify_Do_Not_Rent_Flag_ImportantInformation" , "Status", "Pass");

		}else if (resultFlag.equals("fail")){

			Excel.setCellValBasedOnTcname(path, "CustomerDashBoard","Verify_Do_Not_Rent_Flag_ImportantInformation" , "Status", "Fail");
		}else{
			Excel.setCellValBasedOnTcname(path, "CustomerDashBoard","Verify_Do_Not_Rent_Flag_ImportantInformation" , "Status", "Skip");
		}
