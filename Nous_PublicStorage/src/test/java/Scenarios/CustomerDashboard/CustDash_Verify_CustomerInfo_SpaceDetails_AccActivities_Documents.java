...
public class CustDash_Verify_CustomerInfo_SpaceDetails_AccActivities_Documents extends Browser_Factory {
	...
	{
		return Excel.getCellValue_inlist(path,"CustomerDashBoard","CustomerDashBoard", "CustDash_Verify_CustomerInfo_SpaceDetails_AccActivities_Documents");
	}

	@Test(dataProvider="getLoginData")
	public void CustDash_Verify_CustomerInfo_SpaceDetails_AccActivities_Documents(Hashtable<String, String> tabledata) throws Exception 
	{
		...
		try{
			logger=extent.startTest("CustDash_Verify_CustomerInfo_SpaceDetails_AccActivities_Documents","Customer DashBoard - Verify Customer info, Space Details, Account Activities and Documents Tab");
			...
			Thread.sleep(4000);
			if (cust_accdetails.verify_SpaceDetailsTab()) {
				...
				logger.log(LogStatus.INFO,"Space Details tab is displayed successfully", image);
			}
			...
			if (cust_accdetails.verify_AccountActivitiesTab()) {
				...
				logger.log(LogStatus.INFO, "Account Activities tab is displayed successfully:", image);
			}
			...
			if (cust_accdetails.verify_DocumentsTab()) {
				...
				logger.log(LogStatus.INFO, "Documents tab is displayed successfully", image);
			}
			...
			boolean none = driver.findElements(By.xpath("//div[@class='important-info-title title'][text()='None at this time.']")).size() > 0;
			
			if(none == true){
				String scpath = Generic_Class.takeScreenShotPath();
				Reporter.log(scpath, true);
				String image = logger.addScreenCapture(scpath);
				logger.log(LogStatus.PASS, "None at this Time is displayed successfully in Important Information");
				logger.log(LogStatus.INFO, "None at this Time is displayed successfully in Important Information", image);
			}
				...
				Reporter.log(scpath, true);
				String image = logger.addScreenCapture(scpath);
				logger.log(LogStatus.PASS, "None at this Time is not displayed");
				logger.log(LogStatus.INFO, "None at this Time is not displayed", image);
			}
	...
	public void afterMethod(){
		...
		if(resultFlag.equals("pass")){
			Excel.setCellValBasedOnTcname(path, "CustomerDashBoard","CustDash_Verify_CustomerInfo_SpaceDetails_AccActivities_Documents" , "Status", "Pass");

		}else if (resultFlag.equals("fail")){

			Excel.setCellValBasedOnTcname(path, "CustomerDashBoard","CustDash_Verify_CustomerInfo_SpaceDetails_AccActivities_Documents" , "Status", "Fail");
		}else{
			Excel.setCellValBasedOnTcname(path, "CustomerDashBoard","CustDash_Verify_CustomerInfo_SpaceDetails_AccActivities_Documents" , "Status", "Skip");
		}
