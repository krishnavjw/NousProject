...
public class Verify_Reset_Password_Functionality extends Browser_Factory{
	...
	{
		return Excel.getCellValue_inlist(path,"CustomerDashBoard","CustomerDashBoard", "Verify_Reset_Password_Functionality");
	}

	@Test(dataProvider="getLoginData")
	public void Verify_Reset_Password_Functionality(Hashtable<String, String> tabledata) throws Exception 
	{
		...
		try{
			logger=extent.startTest("Verify_Reset_Password_Functionality","Customer DashBoard - Verify Reset Password Functionality");
	...
	//=================Handling Customer Facing Device================================
			...
			String qry = "Select Top 1 A.AccountNumber "+
					...
					"join websiteuser wu on wu.contactid = c.contactid "+
			...
			if (cust_accdetails.verify_CustomerInfoTab()) {
				...
				logger.log(LogStatus.INFO,"Customer Info tab is displayed successfully", image);
			}
				...
				logger.log(LogStatus.INFO, "Customer Info tab is not displayed", image);
			}
			...
			logger.log(LogStatus.INFO, "Created Object for Customer Info page");
					
			if (custinfo.verify_Username()) {

				Thread.sleep(2000);
				String scpath = Generic_Class.takeScreenShotPath();
				Reporter.log(scpath, true);
				String image = logger.addScreenCapture(scpath);
				logger.log(LogStatus.PASS,"Username is displayed successfully");
				logger.log(LogStatus.INFO,"Username is displayed successfully", image);
			}
				...
				if(resultFlag.equals("pass"))
					resultFlag="fail";
				String image = logger.addScreenCapture(scpath);
				logger.log(LogStatus.FAIL, "Username is not displayed");
				logger.log(LogStatus.INFO, "Username is not displayed", image);
			}
			
			
			Thread.sleep(10000);
			
			if (custinfo.verify_ResetPassword()) {

				Thread.sleep(2000);
				String scpath = Generic_Class.takeScreenShotPath();
				Reporter.log(scpath, true);
				String image = logger.addScreenCapture(scpath);
				logger.log(LogStatus.PASS,"Reset Password Link is displayed successfully");
				logger.log(LogStatus.INFO,"Reset Password Link is displayed successfully", image);
			}
				...
				if(resultFlag.equals("pass"))
					resultFlag="fail";
				String image = logger.addScreenCapture(scpath);
				logger.log(LogStatus.FAIL, "Reset Password Link is not displayed");
				logger.log(LogStatus.INFO, "Reset Password Link is not displayed", image);
			}
			
			//Clicking on reset password
			custinfo.click_ResetButton();
			Thread.sleep(3000);
			logger.log(LogStatus.INFO, "Clicked on Reset password Link Successfully");
			
			boolean success = driver.findElement(By.xpath("//span[contains(text(),'Success')]")).isDisplayed();
			if (success) {
				...
				Reporter.log(scpath, true);
				String image = logger.addScreenCapture(scpath);
				logger.log(LogStatus.PASS,"Email was sent is displayed successfully");
				logger.log(LogStatus.INFO,"Email was sent is displayed successfully", image);
			}
				...
				if(resultFlag.equals("pass"))
					resultFlag="fail";
				String image = logger.addScreenCapture(scpath);
				logger.log(LogStatus.FAIL, "Email was sent is not displayed");
				logger.log(LogStatus.INFO, "Email was sent is not displayed", image);
			}
	...
	public void afterMethod(){
		...
		if(resultFlag.equals("pass")){
			Excel.setCellValBasedOnTcname(path, "CustomerDashBoard","Verify_Reset_Password_Functionality" , "Status", "Pass");

		}else if (resultFlag.equals("fail")){

			Excel.setCellValBasedOnTcname(path, "CustomerDashBoard","Verify_Reset_Password_Functionality" , "Status", "Fail");
		}else{
			Excel.setCellValBasedOnTcname(path, "CustomerDashBoard","Verify_Reset_Password_Functionality" , "Status", "Skip");
		}
