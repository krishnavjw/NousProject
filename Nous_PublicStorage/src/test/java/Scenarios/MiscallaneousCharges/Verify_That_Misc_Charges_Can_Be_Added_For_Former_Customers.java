...
public class Verify_That_Misc_Charges_Can_Be_Added_For_Former_Customers extends Browser_Factory {

	public ExtentTest logger;
	String resultFlag="pass";
	String path = Generic_Class.getPropertyValue("Excelpath");
	String uploadpath=Generic_Class.getPropertyValue("uploadfilepath");

	@DataProvider
	public Object[][] getLoginData() 
	{
		return Excel.getCellValue_inlist(path,"Miscellaneous","Miscellaneous", "Verify_That_Misc_Charges_Can_Be_Added_For_Former_Customers");
	}

	@Test(dataProvider="getLoginData")
	public void Verify_That_Misc_Charges_Can_Be_Added_For_Former_Customers(Hashtable<String, String> tabledata) throws Exception 
	{
		...
		try{
			logger=extent.startTest("Verify_That_Misc_Charges_Can_Be_Added_For_Former_Customers","Verify_That_Misc_Charges_Can_Be_Added_For_Former_Customers");
			...
			String query1="select top 1 a.accountnumber "+
					...
					"and s.sitenumber='"+SiteNumber+"' and c.CustomerTypeID=90 "+
					"group by a.accountnumber, aoi.accountorderitemid, ru.rentalunitnumber "+
					"having sum(clt.amount + clt.discountamount)>0 "+
					"order by 1 ";

			/*String query1="select top 1 accountid from vw_unitdetails where vacatedate is not NULL order by vacatedate desc "; // Updated the Query in 110 Machine
			...
			try {
				...
				driver.findElement(By.xpath("//div[@id='vacated-space-details-grid']//table/tbody/tr[1]/td[1]/a")).click();
				Thread.sleep(2000);
				driver.findElement(By.xpath("//a[contains(text(),' Add Misc Fee')]")).click();
			}
			...
			Thread.sleep(3000);
			mischrge.ledger_note(1,tabledata.get("Notes"));
			Thread.sleep(2000);
			...
			driver.findElement(By.xpath("(//a[contains(text(),'Upload')])[3]")).click();
			Thread.sleep(3000);
			Generic_Class uploadfile= new Generic_Class();
			uploadfile.uploadFileUsingrobot(uploadpath);
			Thread.sleep(20000);
			//String amount3=mischrge.misc_amount(3);
			//double misc_amount3=Double.parseDouble(amount3);
			...
			String amount1=mischrge.misc_amount_abandonedgoodsfeesmall();
			Thread.sleep(3000);
			if(amount1.isEmpty()){
				...
				String scpath=Generic_Class.takeScreenShotPath();
				String image=logger.addScreenCapture(scpath);
				logger.log(LogStatus.FAIL, "Charge Amount is not displayed for Abandoned Goods Small---------------- "+amount1);
				...
				String scpath=Generic_Class.takeScreenShotPath();
				String image=logger.addScreenCapture(scpath);
				logger.log(LogStatus.PASS, "Charge Amount is displayed for Abandoned Goods Small----------------- "+amount1);
			...
			payments.mischarges_dropdown("Abandoned Goods Fee (Medium)", driver);
			Thread.sleep(2000);
			mischrge.Add_charges();
			Thread.sleep(3000);
			...
			mischrge.clk_RemovecleaningBtn();
			Thread.sleep(2000);
			mischrge.clk_RemoveDamageBtn();
			Thread.sleep(3000);
			((JavascriptExecutor)driver).executeScript("window.scrollTo(500,500)");
			Thread.sleep(6000);
			...
			if(Miscellaneous_Chargedisplay){
				...
				String scpath=Generic_Class.takeScreenShotPath();
				String image=logger.addScreenCapture(scpath);
				logger.log(LogStatus.PASS, "customer Dashboard is displayed Miscellaneous_Charge");
				logger.log(LogStatus.INFO, "customer Dashboard is displayed Miscellaneous_Charge",image);
			}else{
				...
				String scpath=Generic_Class.takeScreenShotPath();
				String image=logger.addScreenCapture(scpath);
				logger.log(LogStatus.FAIL, "Miscellaneous_Charge is not displayed ");
				logger.log(LogStatus.INFO, "Miscellaneous_Charge is not displayed ",image);
			}
			js1.executeScript("window.scrollBy(0,200)", "");
			cust_accdetails.VerifyMiscellaneousCharge_click();
			...
			System.out.println("toatlDueNowAmtBeforeMakePymt---"+toatlDueNowAmtBeforeMakePymt);
			System.out.println("toatlDueNowAmtafterMakePymt---"+toatlDueNowAmtafterMakePymt);
			...
			if(total_view_detail_due.equals(toatlDueNowAmtafterMakePymt)){
				...
				String scpath=Generic_Class.takeScreenShotPath();
				String image=logger.addScreenCapture(scpath);
				logger.log(LogStatus.FAIL, "Total due is not displaying Correctly  in Customer Account Dashboard. ");
...
//
//			// double Totalmiscvalue=misc_amount1+misc_amount2+misc_amount3;
//			//	double miscvalue=Double.parseDouble(value);
...
//				logger.log(LogStatus.INFO, "Abandoned Goods Small is  not displaying Correctly ",image);
//			}
			...
			String missvaluequery="Select top 1 clt.amount from CLTransactionMaster cltm "+
					...
					"where cltm.AccountID='"+accnum+"' order by cltm.LastUpdate desc ";   // Updated the Query in 110 Machine
	...
	public void afterMethod(){
		...
		if(resultFlag.equals("pass")){
			Excel.setCellValBasedOnTcname(path, "Miscellaneous","Verify_That_Misc_Charges_Can_Be_Added_For_Former_Customers" , "Status", "Pass");

		}else if (resultFlag.equals("fail")){

			Excel.setCellValBasedOnTcname(path, "Miscellaneous","Verify_That_Misc_Charges_Can_Be_Added_For_Former_Customers" , "Status", "Fail");
		}else{
			Excel.setCellValBasedOnTcname(path, "Miscellaneous","Verify_That_Misc_Charges_Can_Be_Added_For_Former_Customers" , "Status", "Skip");
		}
