...
public class WalkInReservation_NewCustWlkIntoSeeHoldSpaceAndLease extends Browser_Factory{
	...
	{
		return Excel.getCellValue_inlist(path, "Reservation","Reservation","WalkInReservation_NewCustWlkIntoSeeHoldSpaceAndLease");
	}

	@Test(dataProvider="getResvnData")
	public void WalkInReservation_NewCustWlkIntoSeeHoldSpaceAndLease(Hashtable<String, String> tabledata) throws Exception 
	{
    	if(!(tabledata.get("Runmode").equals("Y") && tabledata.get("Reservation").equals("Y")))
		{
			resultFlag="skip";
			throw new SkipException("Skipping the WalkInReservation_NewCustWlkIntoSeeHoldSpaceAndLease test");
		}


		try{

			logger=extent.startTest("WalkInReservation_NewCustWlkIntoSeeHoldSpaceAndLease", "new Customer walk in to see the HOLD space and lease");
			...
			catch(Exception e)
			{
				...
				logger.log(LogStatus.PASS, "User is allowed to enter a reason for cancellation and close the Hold Transaction also done sucessfully and hold space is deleted sucessfully for new customer");
				resultFlag="pass";
			}
			...
			e.getMessage();
			resultFlag="fail";
			Reporter.log("Exception: WalkInReservation_NewCustWlkIntoSeeHoldSpaceAndLease"+e,true);
		}
	...
	public void afterMethod(){
		...
		if(resultFlag.equals("pass")){
			Excel.setCellValBasedOnTcname(path, "Reservation","WalkInReservation_NewCustWlkIntoSeeHoldSpaceAndLease", "Status", "Pass");

		}else if (resultFlag.equals("fail")){

			Excel.setCellValBasedOnTcname(path, "Reservation","WalkInReservation_NewCustWlkIntoSeeHoldSpaceAndLease", "Status", "Fail");
		}else{
			Excel.setCellValBasedOnTcname(path, "Reservation","WalkInReservation_NewCustWlkIntoSeeHoldSpaceAndLease", "Status", "Skip");
		}
