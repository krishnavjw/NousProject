...
public class WalkInReservation_ExtIndCustWlkIntoSeeHoldSpaceAndLease extends Browser_Factory {
	...
	{
		return Excel.getCellValue_inlist(path, "Reservation","Reservation","WalkInReservation_ExtIndCustWlkIntoSeeHoldSpaceAndLease");
	}

	@Test(dataProvider="getResvnData")
	public void WalkInReservation_ExtIndCustWlkIntoSeeHoldSpaceAndLease(Hashtable<String, String> tabledata) throws Exception 
	{
		...
		{
			resultFlag="skip";
			throw new SkipException("Skipping the WalkInReservation_ExtIndCustWlkIntoSeeHoldSpaceAndLease test");
		}


		try{

			logger=extent.startTest("WalkInReservation_ExtIndCustWlkIntoSeeHoldSpaceAndLease", "Existing individual Customer walk in to see the HOLD space and lease");
			...
			pmhomepage.clk_HoldBtnOnInprogessModule(space);
			logger.log(LogStatus.PASS, "click on the hold link on the inprogress module");
			Thread.sleep(5000);
			inprogress.clk_RentRadioBtn();
			logger.log(LogStatus.INFO, "click on Rent radio button");
			Thread.sleep(5000);
			inprogress.clk_ContinueBtn();
			logger.log(LogStatus.INFO, "click on Continue button");

			Leasing_ConfirmSpace leasingPage=new Leasing_ConfirmSpace(driver);
			logger.log(LogStatus.INFO, "Leasing page object is created sucessfully");
			Thread.sleep(5000);
			if(tabledata.get("LeasingPageTitle").contains(leasingPage.getTitle_LeasingPage().trim()))
			{
				String scpath=Generic_Class.takeScreenShotPath();
				String image=logger.addScreenCapture(scpath);
				logger.log(LogStatus.PASS, "Leasing work flow page is displayed");
				logger.log(LogStatus.INFO, "Leasing work flow page is displayed",image);
			}
			else{
				if(resultFlag.equals("pass"))
					resultFlag="fail";
				...
				String scpath=Generic_Class.takeScreenShotPath();
				String image=logger.addScreenCapture(scpath);
				logger.log(LogStatus.FAIL, "Leasing work flow page is displayed is not displayed ");
				logger.log(LogStatus.INFO, "Leasing work flow page is displayed is not displayed ",image);
			...
			logger.log(LogStatus.PASS, "click on the hold link on the inprogress module");
			Thread.sleep(5000);
			inprogress.clk_CancelRadioBtn();
			logger.log(LogStatus.PASS, "click on the cancel radio button");

			Thread.sleep(5000);
			inprogress.sel_DropDownValFromPhNo(tabledata.get("ReasonForCanel"));
			logger.log(LogStatus.PASS, "Select any value from dropdown");
			Thread.sleep(5000);
			inprogress.enter_EmpNumber(tabledata.get("UserName"));
			...
			try
			{
				...
				boolean val=driver.findElement(By.xpath("//td[text()='"+space+"']/preceding-sibling::td/a")).isDisplayed();
				if(val)
				{
					resultFlag="fail";
					logger.log(LogStatus.INFO, "Hold with respect space is not vanished as expected ");
				}
			}
			catch(Exception e)
			{
				Reporter.log("Exception e: "+ e,true);
				logger.log(LogStatus.PASS, "User is allowed to enter a reason for cancellation and close the Hold Transaction also done sucessfully and hold space is deleted sucessfully for existing individual customer ");
				resultFlag="pass";
			}
			...
			e.getMessage();
			resultFlag="fail";
			Reporter.log("Exception: WalkInReservation_ExtIndCustWlkIntoSeeHoldSpaceAndLease"+e,true);
		}
	...
	public void afterMethod(){
		...
		if(resultFlag.equals("pass")){
			Excel.setCellValBasedOnTcname(path, "Reservation","WalkInReservation_ExtIndCustWlkIntoSeeHoldSpaceAndLease", "Status", "Pass");

		}else if (resultFlag.equals("fail")){

			Excel.setCellValBasedOnTcname(path, "Reservation","WalkInReservation_ExtIndCustWlkIntoSeeHoldSpaceAndLease", "Status", "Fail");
		}else{
			Excel.setCellValBasedOnTcname(path, "Reservation","WalkInReservation_ExtIndCustWlkIntoSeeHoldSpaceAndLease", "Status", "Skip");
		}
