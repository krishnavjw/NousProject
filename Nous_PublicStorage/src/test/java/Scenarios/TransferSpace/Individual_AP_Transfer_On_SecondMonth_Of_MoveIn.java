...
public class Individual_AP_Transfer_On_SecondMonth_Of_MoveIn extends Browser_Factory {
	...
	public Object[][] getData() {
		return Excel.getCellValue_inlist(path, "TransferSpace", "TransferSpace","Individual_AP_Transfer_On_SecondMonth_Of_MoveIn");
	}

	@Test(dataProvider = "getData")
	public void Individual_AP_Transfer_On_SecondMonth_Of_MoveIn(Hashtable<String, String> tabledata)
			throws InterruptedException {
		try {

			logger = extent.startTest("Individual_AP_Transfer_On_SecondMonth_Of_MoveIn",
					"Individual_AP_Transfer_On_SecondMonth_Of_MoveIn starts ");
			...
			logger.log(LogStatus.INFO, "Login to Application  with PM  successfully");
			Thread.sleep(5000);
			...
			String sqlQuery = "select aoip.promotionid, p.name, u.unitcost as MonthlyRent,aoipo.unitcost as Insurance,soi.rentdueday,soi.paymentmethodtypeid,u.* "
					...
					+ "where  u.moveindate > getdate()-29 and u.vacatedate is null and u.paidthrudate >getdate() "
					+ "and soi.rentdueday>'1' and u.customertypeid = '90' "
					+ "and p.name = '$1 Special/No Admin Fee' order by u.siteid,u.moveindate";
...
*/
			...
			logger.log(LogStatus.INFO, "Image", image1);
			logger.log(LogStatus.INFO, "Enter existing customer Account Number in Account field successfully "+accountNumber);
			...
			String gateCodeBeforeTransfer = cust.getGateCode();
					
			logger.log(LogStatus.INFO, "Gate Code Before Transfer is:  " + gateCodeBeforeTransfer);

			String promotionOffer = cust.getPromotion();
			logger.log(LogStatus.INFO, "Promotion Offer is:  " + promotionOffer);

			if (promotionOffer.equals("$1 Special/No Admin Fee")) {
				logger.log(LogStatus.PASS, "Promotion " + promotionOffer + " is displayed successfully");
			} else {
	...
	public void afterMethod() {
		...
		if (resultFlag.equals("pass")) {
			Excel.setCellValBasedOnTcname(path, "Transfer", "Individual_AP_Transfer_On_SecondMonth_Of_MoveIn", "Status",
					"Pass");
