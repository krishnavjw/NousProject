...
public class Individual_AP_Transfer_On_SecondDay_Of_MoveIn extends Browser_Factory {
	...
	String rentalUnitIDBeforeTransfer;
	String rentalUnitNumberBeforeTransfer;
	String rentalUnitNumberToBeTransferred;
	String product_siteid;

	@DataProvider
	public Object[][] getData() {
		return Excel.getCellValue_inlist(path, "TransferSpace", "TransferSpace",
				"Individual_AP_Transfer_On_SecondDay_Of_MoveIn");
	}

	@Test(dataProvider = "getData")
	public void Individual_AP_Transfer_On_SecondDay_Of_MoveIn(Hashtable<String, String> tabledata)
			throws InterruptedException {
		try {

			logger = extent.startTest("Individual_AP_Transfer_On_SecondDay_Of_MoveIn",
					"Individual_AP_Transfer_On_SecondDay_Of_MoveIn starts ");
			...
			String sqlQuery = "select aoip.promotionid, p.name, u.unitcost as MonthlyRent,aoipo.unitcost as Insurance,soi.rentdueday,soi.paymentmethodtypeid,u.* "
					...
					+ "and u.customertypeid = '90' and p.promotionid = '84' " + "order by u.siteid,u.moveindate";
			...
			try {
				...
				rentalUnitNumberBeforeTransfer = results.get(15);
				logger.log(LogStatus.INFO,
						"Rental Unit Number fetched from DB before transfer is: " + rentalUnitNumberBeforeTransfer);
				System.out.println("Rental Unit Number before transfer is:   " + rentalUnitNumberBeforeTransfer);

				rentalUnitIDBeforeTransfer = results.get(16);
				logger.log(LogStatus.INFO,
						"Rental Unit ID fetched from DB before transfer is: " + rentalUnitIDBeforeTransfer);
				System.out.println("Rental Unit Id before transfer is:   " + rentalUnitIDBeforeTransfer);

			} catch (Exception e) {
				logger.log(LogStatus.INFO, "No Details are fetched from Database");
			}
			...
			if (!(current_siteid.equals(siteid_tobeset))) {
				...
				siteid_tobeset = current_siteid;
			}

			/*
			 * Fetching product site id & rental unit spaces available in the
			 * current site id with rental status type id as 59
			 * 
			 */

			String sqlQuery2 = "select top 1 ps.siteid,ru.* from rentalunit ru "
					+ "join productsite ps on ru.productsiteid=ps.productsiteid " + "where ps.siteid = '"
					+ siteid_tobeset + "' and ru.rentalstatustypeid = '59'";

			try {

				ArrayList<String> results = DataBase_JDBC.executeSQLQuery_List(sqlQuery2);
				System.out.println(results);

				product_siteid = results.get(3);
				logger.log(LogStatus.INFO, "Product Site is:  " + product_siteid);
				System.out.println("Product Site is:   " + product_siteid);

				rentalUnitNumberToBeTransferred = results.get(4);
				logger.log(LogStatus.INFO,
						"Rental Unit Number fetched from DB before transfer is: " + rentalUnitNumberToBeTransferred);
				System.out.println("Rental Unit Number before transfer is:   " + rentalUnitNumberToBeTransferred);

			} catch (Exception e) {
				logger.log(LogStatus.INFO, "No Details are fetched from Database");
			}

			String sqlQuery3 = "insert into promotionproductsite "
					+ "(lastupdate,promotionid, effectivedate,employeeid,productsiteid) "
					+ "values (getutcdate(),'132','2016-10-01','-1','" + product_siteid + "')";

			DataBase_JDBC.executeSQLQuery(sqlQuery3);
	...
	public void afterMethod() {
		...
		if (resultFlag.equals("pass")) {
			Excel.setCellValBasedOnTcname(path, "Transfer", "Individual_AP_Transfer_On_SecondDay_Of_MoveIn", "Status",
					"Pass");
