...
public class TaskType3_CountPettyCashPM_InCompleteTask extends Browser_Factory {
	...
	{
		return Excel.getCellValue_inlist(path,"TaskManagement","TaskManagement", "TaskType3_CountPettyCashPM_InCompleteTask");
	}

	@Test(dataProvider="getLoginData")
	public void TaskType3_CountPettyCashPM_InCompleteTask(Hashtable<String, String> tabledata) throws Exception 
	{

		if(!(tabledata.get("Runmode").equals("Y") && tabledata.get("TaskManagement").equals("Y")))
		{
			resultFlag="skip";
			logger.log(LogStatus.SKIP, "TaskType3_CountPettyCashPM_InCompleteTask is Skipped");
			throw new SkipException("Skipping the test ");
		}


		try{
			logger=extent.startTest("TaskType3_CountPettyCashPM_InCompleteTask", "Task Type 3 - Count Petty Cash PM InComplete Task");
			testcaseName = tabledata.get("TestCases");
			Reporter.log("Test case started: " +testcaseName, true);
			
			wait = new WebDriverWait(driver, 60);
			
			String sqlQuery = "select distinct s.sitenumber,t.* from Taskmaster TM with(nolock) "+
					"join groupsite gs with(nolock) on gs.groupid=tm.propertygroupid "+
					"join site s with(nolock) on s.siteid=gs.siteid "+
					"join task t with(nolock) on t.locationcode=s.sitenumber "+
					"left join taskitem ti with(nolock) on ti.taskid=t.id "+
					"where t.tasktypeid=7029 "+
					"and ti.taskid is null "+
					"and t.createdt <getutcdate() and t.completiondt is null "+
					"and t.duedt > getutcdate() ";
			...
			if(homepage.isexistingCustomerModelDisplayed()){
				...
				String scpath=Generic_Class.takeScreenShotPath();
				String image=logger.addScreenCapture(scpath);
				logger.log(LogStatus.FAIL, "user is not navigated to PM dashboard");
				logger.log(LogStatus.INFO, "img",image);
			}
		  ...
		  List<WebElement> listTask = driver.findElements(By.xpath("//div[@id='task-grid']//div//table//tbody//tr//td/a"));
            ...
            for(int i=0;i<size;i++){
	            if(listTask.get(i).getText().contains("Count Petty Cash - Evening")){
	                        driver.findElement(By.xpath("//div[@id='task-grid']//div//table//tbody//tr//td/a[contains(text(),'Count Petty Cash - Evening')]")).click();
	                        break;
	            }
	            if(i%2==0){
			...
			Thread.sleep(15000);
			logger.log(LogStatus.PASS, "Clicked on Count Petty Cash - Evening Link successfully");
			...
			boolean title = driver.findElement(By.xpath("//h3[contains(text(),'Count Petty Cash - Evening')]")).isDisplayed();
			if(title){
				String scpath=Generic_Class.takeScreenShotPath();
				String image=logger.addScreenCapture(scpath);
				logger.log(LogStatus.PASS, "Count Petty Cash - Evening Page is displayed Successfully");
				logger.log(LogStatus.INFO, "img",image);
			}
			else{
				String scpath=Generic_Class.takeScreenShotPath();
				String image=logger.addScreenCapture(scpath);
				logger.log(LogStatus.FAIL, "Count Petty Cash - Evening Page is not displayed");
				logger.log(LogStatus.INFO, "img",image);
			}
			...
			for (int i = size1; i > 0; i--) {
				...
				if (listTaskTitle.get(i-1).getText().trim().equalsIgnoreCase(viewtaskTitle.trim())) {
	...
	public void afterMethod(){
		...
		if(resultFlag.equals("pass")){
			...
			Excel.setCellValBasedOnTcname(path, "TaskManagement","TaskType3_CountPettyCashPM_InCompleteTask" , "Status", "Pass");

		}else if (resultFlag.equals("fail")){

			Excel.setCellValBasedOnTcname(path, "TaskManagement","TaskType3_CountPettyCashPM_InCompleteTask" , "Status", "Fail");
		}else{
			Excel.setCellValBasedOnTcname(path, "TaskManagement","TaskType3_CountPettyCashPM_InCompleteTask" , "Status", "Skip");
		}
