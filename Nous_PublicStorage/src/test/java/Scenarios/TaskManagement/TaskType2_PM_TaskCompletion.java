...
public class TaskType2_PM_TaskCompletion extends Browser_Factory{
	...
	{
		return Excel.getCellValue_inlist(path,"TaskManagement","TaskManagement", "TaskType2_PM_To_Complete_The_Task");
	}

	@Test(dataProvider="getLoginData")
	public void TaskType2_PM_To_Complete_The_Task(Hashtable<String, String> tabledata) throws Exception 
	{

		if(!(tabledata.get("Runmode").equals("Y") && tabledata.get("TaskManagement").equals("Y")))
		{
			resultFlag="skip";
			logger.log(LogStatus.SKIP, "TaskType2_PM_To_Complete_The_Task is Skipped");
			throw new SkipException("Skipping the test ");
		}


		try{
			logger=extent.startTest("TaskType2_PM_To_Complete_The_Task", "Task Type 2 - PM to complete the task");
			testcaseName = tabledata.get("TestCases");
			Reporter.log("Test case started: " +testcaseName, true);
			
			wait = new WebDriverWait(driver, 60);
			
			String sqlQuery = "select distinct top 1 s.siteid,s.sitenumber,ti.taskid, ti.content,t.* from Taskmaster TM with(nolock) "+
			...
			try { 
				...
				String Tasknamefromdb = results.get(5);
		...
		/*	Thread.sleep(5000);
			...
			logger.log(LogStatus.INFO, "Title fetched from database: "+Tasknamefromdb);
		   ...
		   List<WebElement> listTask = driver.findElements(By.xpath("//div[@id='task-grid']//div//table//tbody//tr//td/a"));
			...
			taskpage.Select_Checkbx();
			logger.log(LogStatus.PASS, "Select All check box is clicked");
			Thread.sleep(4000);
	...
	public void afterMethod(){
		...
		if(resultFlag.equals("pass")){
			...
			Excel.setCellValBasedOnTcname(path, "TaskManagement","TaskType2_PM_To_Complete_The_Task" , "Status", "Pass");

		}else if (resultFlag.equals("fail")){

			Excel.setCellValBasedOnTcname(path, "TaskManagement","TaskType2_PM_To_Complete_The_Task" , "Status", "Fail");
		}else{
			Excel.setCellValBasedOnTcname(path, "TaskManagement","TaskType2_PM_To_Complete_The_Task" , "Status", "Skip");
		}
