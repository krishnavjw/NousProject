...
public class TaskType3_InComplete_CountPettyCash_AMTask extends Browser_Factory{
	...
	public Object[][] getLoginData() {
		return Excel.getCellValue_inlist(path, "TaskManagement", "TaskManagement", "TaskType3_InComplete_CountPettyCash_AMTask");
	}
	...
	public void TaskType3_Complete_CountPettyCash_AMTask(Hashtable<String, String> tabledata) throws Exception {
		...
		if (!(tabledata.get("Runmode").equals("Y") && tabledata.get("TaskManagement").equals("Y"))) {
			resultFlag = "skip";
			logger.log(LogStatus.SKIP, "TaskType3_InComplete_CountPettyCash_AMTask is Skipped");
			throw new SkipException("Skipping the test ");
		}

		try {
			logger = extent.startTest("TaskType3_InComplete_CountPettyCash_AMTask", "Task Type 3 - InComplete CountPettyCash AMTask");
...
//===========================================
		...
		listTaskTitle1.get(i));
			...
			Thread.sleep(5000);
			logger.log(LogStatus.PASS, "The task is completed and the status is :" + Status);
			Thread.sleep(6000);
			 scpath = Generic_Class.takeScreenShotPath();
			 image = logger.addScreenCapture(scpath);
			logger.log(LogStatus.PASS, "img", image);
